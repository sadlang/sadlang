...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
    package=")" << config_.packageName
                 << R"("
    android:versionCode=")"
                 << config_.versionCode << R"("
    android:versionName=")"
                 << config_.version << R"(">
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            if (i + 1 < argc)
                options.sdkPath = argv[++i];
        }
        else if (arg == "--lib")
        {
            if (i + 1 < argc)
                options.nativeLibPath = argv[++i];
        }
