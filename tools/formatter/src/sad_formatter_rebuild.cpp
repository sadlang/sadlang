...
// ============================================================================
// sad_formatter_rebuild.cpp — إعادة بناء الكود ودوال مساعدة
// Rebuild & helper functions for SadFormatter
// ============================================================================
//
// (AR) يحتوي على: rebuild() (التجميع من الرموز)، sortAndGroupImports،
//      wrapLine، findBreakPoint، ودوال التطبيع المساعدة (splitLines،
//      joinLines، normalizeLineEndings، lineLength، ...).
//      تم استخراج هذا الملف من sad_formatter.cpp وفقاً لقاعدة CW-05
//      (الحد الأقصى لحجم الملف 800 سطر).
// (EN) Contains: rebuild() (token assembly), sortAndGroupImports, wrapLine,
//      findBreakPoint, and small text helpers. Split off from sad_formatter.cpp
//      per project rule CW-05 (≤800 LOC per file).
// ============================================================================
...
namespace Sad
{
    namespace Format
    {
        ...
        // ============================================================================
        // rebuild — إعادة بناء الكود من الرموز مع المسافات والمسافة البادئة
        // ============================================================================
        ...
        std::string SadFormatter::rebuild(const std::vector<FmtToken> &tokens)
        {
            ...
            (void)inImportBlock; // (AR) محجوزة لتمييز كتل الاستيراد مستقبلاً
            ...
            for (size_t i = 0; i < tokens.size(); ++i)
            {
                ...
                case FmtTokenType::WHITESPACE:
                    // تجاهل المسافات في بداية السطر — سننشئ مسافة بادئة
                    if (!startOfLine)
                    {
                        // مسافة واحدة فقط
                        out << " ";
                ...
                case FmtTokenType::IMPORT_KEYWORD:
                {
                    ...
                    if (TS::isInterBlockKeyword(tok.value))
                    {
                    ...
                    else if (TS::isBlockCloserKeyword(tok.value))
                    {
                        // نهاية
                    ...
                    else
                    {
                        ...
                        out << tok.value;
                        if (TS::isBlockOpenerKeyword(tok.value))
                        {
                            indentLevel++;
                        }
                    }
                    ...
                    {
                        // لا تضع مسافة إذا كان التالي قوس فتح ولغة ص لا تحتاجه
                        if (next->value != "(" || options_.spaceBeforeParenInDef ||
                            !TS::isBlockOpenerKeyword(tok.value))
                        {
                            out << " ";
                        }
                    }
        ...
        std::string SadFormatter::sortAndGroupImports(const std::string &source) const
        {
            ...
            if (hasFollowing)
            {
                ...
                while (nextNonEmpty < lines.size() &&
                       lines[nextNonEmpty].find_first_not_of(" \t\r\n") == std::string::npos)
                {
                    nextNonEmpty++;
                }
        ...
        std::string SadFormatter::wrapLine(const std::string &line, int currentIndent) const
        {
            ...
            // لفّ الجزء الثاني إذا طوله أيضاً يتجاوز الحد
        ...
        int SadFormatter::findBreakPoint(const std::string &line, int maxCol) const
        {
            ...
            // أولوية الكسر: بعد الفاصلة، ثم قبل العامل، ثم بعد القوس
        ...
        int formatter_main(int argc, char *argv[])
        {
            ...
            if (fs::is_directory(path))
            {
                ...
                if (dryRun)
                    std::cout << "  (محاكاة — لم تُحفظ تغييرات)\n";
            ...
            if (fs::is_regular_file(path))
            {
                ...
                if (result.success())
                {
                    if (result.changed)
                    {
                        std::cout << "✓ تم التنسيق: " << path << " (" << result.linesChanged << " سطر)\n";
                    }
                    else
                    {
