...
struct AnalyzedSymbol {
    ...
    // رمزٌ من الخطة الاحتياطية بـ regex (collect_symbols_from_ast) لا من عبور AST. الخطة
    // مسحٌ سطحيّ للأسطر لا يميّز النطاق، فتُسنَد محليّات الأجسام خطأً إلى العمق 0. لذا تُستبعَد
    // هذه الرموز من كشف التعريفات المكرّرة (ص-ت١٠٢) الذي يثق بمعلومة نطاق AST الدقيقة وحدها؛
    // وإلّا لأنتج محلّيٌّ يظلّل اسمًا علويًّا مختلف النوع تحذير «تعريف مكرر» كاذبًا. [مراجعة Amelia]
    bool is_fallback = false;        // مجموع بـ regex الاحتياطيّ لا بعبور AST
};
