...
async function loadStats() {
    ...
    if (data) {
        ...
        if (el('statPackages')) el('statPackages').textContent = formatNumber(s.total_packages || 0);
        if (el('statDownloads')) el('statDownloads').textContent = formatNumber(s.total_downloads || 0);
        if (el('statUsers'))    el('statUsers').textContent = formatNumber(s.total_users || 0);
    }
}
...
async function loadPackageSection(endpoint, containerId) {
    ...
    if (data) {
        // API يرجع مفاتيح مختلفة: featured, popular, recent, packages
        const pkgs = data.packages || data.featured || data.popular || data.recent || (Array.isArray(data) ? data : []);
        if (pkgs.length > 0) {
            container.innerHTML = '';
            pkgs.forEach(pkg => {
...
// ═══════════════════════════════════════════════════════════════════════════════
// التفضيلات (Stars) والتقييمات (Reviews) والبلاغات
// ═══════════════════════════════════════════════════════════════════════════════
...
async function toggleStar() {
    ...
    const data = await apiCall(`/packages/${encodeURIComponent(currentPackageName)}/star`, {
        method: 'POST',
    });
    ...
    if (data) {
        ...
        if (data.starred) {
            ...
            if (text) text.textContent = 'مُفضّل';
            document.getElementById('starBtn')?.classList.add('starred');
        } else {
            if (icon) icon.textContent = '☆';
            if (text) text.textContent = 'تفضيل';
            document.getElementById('starBtn')?.classList.remove('starred');
        }
        if (count) count.textContent = formatNumber(data.star_count);
    }
}

/**
 * تحميل التقييمات لحزمة معينة
 */
async function loadReviews(packageName) {
    const data = await apiCall(`/packages/${encodeURIComponent(packageName)}/reviews`);
    const list = document.getElementById('reviewsList');
    const summary = document.getElementById('reviewSummary');

    if (!list) return;

    // تحديث عدد النجوم في الشريط الجانبي
    if (data && data.star_count !== undefined) {
        const countEl = document.getElementById('starCount');
        if (countEl) countEl.textContent = formatNumber(data.star_count);
    }

    // عرض ملخص التقييمات
    if (summary && data) {
        summary.innerHTML = `<span>${data.review_count || 0} تقييم</span>`;
    }

    // إظهار نموذج التقييم للمستخدمين المسجلين
    const token = localStorage.getItem('sad_token');
    const formWrapper = document.getElementById('reviewFormWrapper');
    if (formWrapper && token) {
    ...
    if (data && data.reviews && data.reviews.length > 0) {
        ...
        data.reviews.forEach(review => {
            ...
            card.innerHTML = `
                <div class="review-header">
                    ...
                    ${review.is_edited ? '<span class="review-edited">(معدّل)</span>' : ''}
                </div>
        ...
        list.innerHTML = '<div class="empty-state"><p>لا توجد تقييمات بعد. كن أول من يقيّم هذه الحزمة!</p></div>';
    }
}

/**
 * تهيئة أزرار النجوم في نموذج التقييم
 */
...
async function submitReview() {
    if (selectedRating < 1 || selectedRating > 5) {
        showToast('يرجى اختيار تقييم بالنجوم (١-٥)');
        return;
    }
    if (!currentPackageName) return;
    ...
    const data = await apiCall(`/packages/${encodeURIComponent(currentPackageName)}/reviews`, {
        method: 'POST',
        body: JSON.stringify({ rating: String(selectedRating), title, body }),
    });

    if (data && data.success) {
        showToast('✓ تم حفظ تقييمك');
        // إعادة تحميل التقييمات
        loadReviews(currentPackageName);
        // مسح النموذج
        if (document.getElementById('reviewTitle')) document.getElementById('reviewTitle').value = '';
        if (document.getElementById('reviewBody')) document.getElementById('reviewBody').value = '';
        selectedRating = 0;
        document.querySelectorAll('#starRatingInput .star-btn').forEach(s => s.textContent = '☆');
    } else {
...
function showReportForm() {
    const token = localStorage.getItem('sad_token');
    if (!token) {
        showToast('يجب تسجيل الدخول لتقديم بلاغ');
        return;
    }

    const reason = prompt('سبب البلاغ:\n(مثال: محتوى مسيء، برمجيات خبيثة، انتهاك حقوق)');
    if (!reason) return;

    const details = prompt('تفاصيل إضافية (اختياري):') || '';

    apiCall(`/packages/${encodeURIComponent(currentPackageName)}/report`, {
        method: 'POST',
        body: JSON.stringify({ reason, details }),
    }).then(data => {
        if (data && data.success) {
            showToast('✓ تم تقديم البلاغ بنجاح. شكراً لمساعدتك!');
        } else {
            showToast('⚠ فشل تقديم البلاغ');
        }
    });
}

// ═══════════════════════════════════════════════════════════════════════════════
// اقتراحات البحث (Search Autocomplete)
// ═══════════════════════════════════════════════════════════════════════════════
...
function initSearchSuggestions() {
    ...
    searchInputs.forEach(input => {
        ...
        let dropdown = input.parentElement.querySelector('.search-suggestions');
        if (!dropdown) {
        ...
        input.addEventListener('input', () => {
            ...
            suggestTimeout = setTimeout(async () => {
                const data = await apiCall(`/suggest?q=${encodeURIComponent(q)}&limit=6`);
                if (data && data.suggestions && data.suggestions.length > 0) {
                    dropdown.innerHTML = data.suggestions.map(s =>
                        `<a class="suggestion-item" href="/search.html?q=${encodeURIComponent(s)}">${escapeHtml(s)}</a>`
                    ).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (path === '/' || path === '/index.html' || path === '') {
        ...
        // ═══ صفحة الحزمة ═══
        const params = getUrlParams();
        currentPackageName = params.get('name');
        loadPackageDetails();
