    ...
    namespace driver
    {
        ...
        std::string findAndroidNdk()
        {
            ...
            if (const char *sdk = std::getenv("ANDROID_SDK_ROOT"))
            {
                ...
                if (fs::exists(ndkBundle))
                    return ndkBundle.string();
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
    package=")" << config.packageName
               << R"("
    android:versionCode=")"
               << config.versionCode << R"("
    android:versionName=")"
               << config.versionName << R"(">

    <uses-sdk
        android:minSdkVersion=")"
               << config.minSdkVersion << R"("
        android:targetSdkVersion=")"
               << config.targetSdkVersion << R"(" />
