    ...
    namespace driver
    {
        ...
        bool CompilerDriver::run_frontend(const std::string &file)
        {
            ...
            // ════════════════════════════════════════════════════════════════════════
            {
                ...
                // ════════════════════════════════════════════════════════════════════
                // (AR) المرحلة 0.0-ب: «--حجم-الكومة» بلا وضعٍ حرّ
                // ════════════════════════════════════════════════════════════════════
                // هنا — لا في وزم سطر الأوامر — لأنّ الوضعَ الحرَّ يُفعَّل من طريقين:
                // العلمُ «--حرّ»، وسمةُ المصدر «#![بلا_مكتبة_قياسية]». والثانيةُ لم
                // تُقرأ إلّا قبل أسطر. الفحصُ قبلها كان يرفض نواةً حرّةً فعلًا
                // ويصفُها بالمستضافة، فيصير العلمُ الجديد عديمَ الفائدة لكلِّ نواةٍ
                // تكتفي بالسمة.
                // (EN) Checked here, not at CLI parse time: freestanding may be
                // activated by a source attribute, which is only known now.
                // ════════════════════════════════════════════════════════════════════
                if (options_.freestanding_heap_mib != 0 && !options_.freestanding)
                {
                    diagnostics_.report_error(::sad::cli::messages::HeapSizeNeedsFreestanding);
                    return false;
                }
