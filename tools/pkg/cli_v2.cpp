...
static std::filesystem::path get_config_dir() {
    ...
    if (appdata) return std::filesystem::path(appdata) / "sad-lang";
    return std::filesystem::path("C:\\ProgramData\\sad-lang");
#else
    const char* home = std::getenv("HOME");
    if (home) return std::filesystem::path(home) / ".config" / "sad-lang";
...
#ifdef _WIN32
    const char* appdata = std::getenv("LOCALAPPDATA");
    if (appdata) return std::filesystem::path(appdata) / "sad-lang" / "cache";
    return std::filesystem::path("C:\\ProgramData\\sad-lang\\cache");
#else
    const char* home = std::getenv("HOME");
    if (home) return std::filesystem::path(home) / ".cache" / "sad-lang";
...
static int cmd_install(const std::string& pkg_arg = "") {
    ...
    if (ok == static_cast<int>(resolved.size())) {
        ...
        if (!pkg_arg.empty()) save_package(pkg);
    } else {
        print_warning("Installed " + std::to_string(ok) + "/"
...
static int cmd_verify() {
    ...
    if (issues == 0) print_success("All OK!");
    else print_warning(std::to_string(issues) + " issues");
    return issues > 0 ? 1 : 0;
}
...
static int cmd_publish(bool local_mode) {
    ...
    if (local_mode) {
        ...
        auto dest = (ad ? std::filesystem::path(ad) : std::filesystem::path("C:\\ProgramData"))
                    / "sad" / "packages" / pkg.name / pkg.version.to_string();
#else
        const char* hm = std::getenv("HOME");
        auto dest = (hm ? std::filesystem::path(hm) / ".sad" : std::filesystem::path("/usr/local/share/sad"))
                    / "packages" / pkg.name / pkg.version.to_string();
#endif
...
static int cmd_config(const std::string& key, const std::string& val) {
    ...
    if (val.empty()) {
        if (key == "registry") std::cout << get_registry_url() << "\n";
        else if (key == "token") std::cout << (get_saved_token().empty() ? "(none)" : "(set)") << "\n";
        else {
    ...
    if (std::filesystem::exists(cp)) {
        ...
        while (std::getline(f, line)) {
            ...
            if (eq != std::string::npos) {
                ...
                while (!k2.empty() && k2.back() == ' ') k2.pop_back();
...
static int cmd_doctor() {
    ...
    if (issues == 0) print_success("Environment OK!");
    else print_warning(std::to_string(issues) + " issues found.");
    return issues > 0 ? 1 : 0;
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (cmd == "install") {
            ...
            if (argc > 2) {
                a = argv[2];
                if (argc > 3) a += " " + std::string(argv[3]);
            }
            return cmd_install(a);
        }

        if (cmd == "add") {
            if (argc < 3) { print_error("Package name required."); return 1; }
            std::string s = argv[2];
            if (argc > 3) s += " " + std::string(argv[3]);
            return cmd_add(s);
        }
        ...
        if (cmd == "update" || cmd == "upgrade") return cmd_update();
        if (cmd == "list" || cmd == "ls") return cmd_list();
        if (cmd == "verify") return cmd_verify();

        if (cmd == "search") {
            if (argc < 3) { print_error("Query required."); return 1; }
            std::string q = argv[2];
            for (int i = 3; i < argc; i++) q += " " + std::string(argv[i]);
            return cmd_search(q);
        }
        ...
        if (cmd == "publish") {
            ...
            for (int i = 2; i < argc; i++)
                if (std::string(argv[i]) == "--local") loc = true;
