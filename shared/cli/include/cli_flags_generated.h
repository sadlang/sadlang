...
inline constexpr FlagSpec kFlags[] = {
    ...
    { "--حجم-الكومة", FlagKind::Value, FlagAction::HeapSize, "ميغابايت", "حجم كومة الوضع الحرّ (يتطلّب «--حرّ»؛ الافتراضيّ 4 على معدن عارٍ و512 في نطاق المستخدم)", "Freestanding heap size (requires «--حرّ»; default 4 bare-metal, 512 userspace)", true, false, false },
    ...
    { "--خلفية-أصلية", FlagKind::Bool, FlagAction::NativeBackend, "", "التوليد بالخلفيّة الأصليّة بلا LLVM (SIR ⇒ ELF64 ساكن)", "Generate via the native no-LLVM backend (SIR ⇒ static ELF64)", true, false, false },
 ...
 */
inline constexpr std::size_t kFlagCount = 68;
...
namespace messages {
    ...
    inline constexpr const char* HeapSizeInvalid = ": حجم الكومة يجب أن يكون عددًا صحيحًا موجبًا بالميغابايت / heap size must be a positive integer in megabytes: ";
    inline constexpr const char* HeapSizeNeedsFreestanding = "«--حجم-الكومة» يتطلّب وضعًا حرًّا: في الوضع المستضاف تدير الكومةَ مكتبةُ النظام لا المترجم. أضف العلم «--حرّ» أو السمة «#![بلا_مكتبة_قياسية]» في رأس المصدر إن قصدت بناءً حرًّا، أو احذف العلم / «--حجم-الكومة» requires freestanding mode: in hosted mode the heap is managed by the system library, not the compiler. Add the «--حرّ» flag or the «#![no_std]» source attribute if you meant a freestanding build, or drop the flag";
