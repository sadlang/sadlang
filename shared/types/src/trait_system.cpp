...
TraitImpl::TraitImpl(SadTypePtr forType, TraitPtr trait)
    : forType_(forType)
    , trait_(trait)
{
}
...
bool TraitBound::isSatisfiedBy(SadTypePtr type, TraitRegistry& registry) const {
...
TraitImplPtr TraitRegistry::findImpl(SadTypePtr type, TraitPtr trait) const {
...
bool TraitRegistry::doesImplement(SadTypePtr type, TraitPtr trait) const {
    return findImpl(type, trait) != nullptr;
}

std::vector<TraitPtr> TraitRegistry::getImplementedTraits(SadTypePtr type) const {
    std::vector<TraitPtr> result;
...
std::vector<SadTypePtr> TraitRegistry::getImplementingTypes(TraitPtr trait) const {
