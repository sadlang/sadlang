...
// (AR) الهرمُ الواحدُ للأنواع — منه SadTypePtr.
// (EN) The single type hierarchy — source of SadTypePtr.
...
struct TraitMethod {
    ...
    std::vector<SadTypePtr> paramTypes; // أنواع المعاملات
    SadTypePtr returnType;             // نوع الإرجاع
...
public:
     ...
     */
    TraitImpl(SadTypePtr forType, TraitPtr trait);
    
    /**
     * @brief الحصول على النوع
     */
    SadTypePtr getType() const { return forType_; }
...
public:
     ...
     */
    bool isSatisfiedBy(SadTypePtr type, class TraitRegistry& registry) const;
...
public:
     ...
     */
    TraitImplPtr findImpl(SadTypePtr type, TraitPtr trait) const;
    
    /**
     * @brief هل النوع ينفذ السمة؟
     */
    bool doesImplement(SadTypePtr type, TraitPtr trait) const;
    
    /**
     * @brief الحصول على كل السمات التي ينفذها نوع
     */
    std::vector<TraitPtr> getImplementedTraits(SadTypePtr type) const;
    
    /**
     * @brief الحصول على كل الأنواع التي تنفذ سمة
     */
    std::vector<SadTypePtr> getImplementingTypes(TraitPtr trait) const;
