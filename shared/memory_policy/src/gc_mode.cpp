    ...
    namespace Memory
    {
        ...
        bool MemoryModeManager::setModeFromFlag(const std::string &flag)
        {
            ...
            case MemoryMode::Hybrid:
                // (AR) مهجور: --hybrid حُذف. يُعامَل كـ gcDefaults() للتوافق الخلفي
                // (EN) Deprecated: --hybrid removed. Treated as gcDefaults() for backward compat
                settings_ = MemoryModeSettings::gcDefaults();
                break;
        ...
        std::optional<MemoryMode> MemoryModeManager::parseCommandLineFlag(const std::string &flag) const
        {
            ...
            //      استخدم --gc بدلاً منها. أي استخدام يُرجع std::nullopt (يُرفض كعَلَم غير معروف).
            // (EN) Phase E-3: --dev/--development/--hybrid/--mixed removed permanently.
            //      Use --gc instead. Any usage returns std::nullopt (rejected as unknown flag).
            if (flag == "--gc" || flag == "--gc-mode")
            {
            ...
            {
                return MemoryMode::Development; // (AR) وضع التعلم = Development مع teacherMode مفعَّل
            }
        ...
        std::string gcStrategyToArabic(GCStrategy strategy)
        {
            switch (strategy)
            {
            case GCStrategy::None:
                return "بدون (ملكية صرفة)";
            case GCStrategy::ReferenceCounting:
                return "عد المراجع";
            case GCStrategy::AtomicReferenceCounting:
                return "عد المراجع الذري";
            case GCStrategy::Tracing:
                return "تتبع (Mark & Sweep)";
