...
namespace Sad
{
    namespace Errors
    {
        ...
        private:
            ...
            ExplanationLevel explanationLevel_ = ExplanationLevel::NORMAL; ///< (AR) مستوى الشرح --explain / (EN) --explain level
            bool catalogInitialized_ = false;                              ///< (AR) هل تم تحميل القوالب الافتراضية / (EN) Whether defaults loaded
        ...
        public:
            ...
            /**
             * @brief (AR) يهيّئ القوالب الافتراضية في ErrorCatalog (يُستدعى مرة واحدة من main)
             *        (EN) Initializes default templates in ErrorCatalog (call once from main)
             */
            void initializeDefaults();

            /**
             * @brief (AR) يحدّد مستوى الشرح (--explain)
             *        (EN) Sets explanation level (--explain)
             */
            void setExplanationLevel(ExplanationLevel level);

            /**
             * @brief (AR) يرجع مستوى الشرح الحالي
             *        (EN) Returns current explanation level
             */
            ExplanationLevel getExplanationLevel() const;

            /**
             * @brief (AR) يرجع اللغة الحالية للرسائل
             *        (EN) Returns current message language
             */
            Language getLanguage() const;

            /**
             * @brief (AR) يبني نصاً ثنائي اللغة من القالب — مساعد لتركيب الرسائل
             *        (EN) Builds bilingual string from template — helper for message assembly
             *
             * (AR) أداة مستخدَمة في مسار `throwRuntime` لبناء النص النهائي من قالب الكتالوج.
             * (EN) Helper used by `throwRuntime` to build final text from catalog template.
             *      at `throw <Error>(...)` sites inside the interpreter without breaking
             *      the existing hierarchy.
             *
             * @return نص بصيغة "(AR) ... / (EN) ..." حسب getLanguage() و getExplanationLevel()
             */
            std::string buildBilingualMessage(ErrorCode code,
                                              const RenderContext &ctx) const;

            /**
             * @brief (AR) يُسجّل خطأ في الـbatch مستخدماً Catalog (للمترجم)
             *        (EN) Records an error into the batch using Catalog (for compiler)
             *
             * (AR) يبني الرسالة ar/en من القالب ويستدعي reportError القديمة لتجميعها.
             *      تستهلكه طبقة المترجم بدل diagnostics_.report_error("...").
             * (EN) Builds ar/en message from template and forwards to legacy reportError.
             *      Consumed by compiler layer instead of diagnostics_.report_error("...").
             */
            void reportFromCatalog(ErrorCode code,
                                   const SourceLocation &location,
                                   const RenderContext &ctx = {});

            /**
             * @brief (AR) نسخة تحذير من reportFromCatalog
             *        (EN) Warning variant of reportFromCatalog
             */
            void reportWarningFromCatalog(ErrorCode code,
                                          const SourceLocation &location,
                                          const RenderContext &ctx = {});

            /**
             * @brief (AR) يطبع جميع التشخيصات إلى ostream عبر Formatter الموحَّد
             *        (EN) Flushes all diagnostics to ostream via unified Formatter
             */
