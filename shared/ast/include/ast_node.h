...
public:
    ...
    /**
     * @brief Get the type of this expression / الحصول على نوع التعبير
     * @return The data type / نوع البيانات
     * 
     * Returns the type that this expression evaluates to.
     * Used by the semantic analyzer for type checking.
     * 
     * يُرجع النوع الذي يُقيَّم إليه هذا التعبير.
     * يُستخدم من قبل المحلل الدلالي للتحقق من الأنواع.
     * 
     * @example
     * @code{.cpp}
     * auto expr = std::make_unique<LiteralExpr>(Token(TokenType::NUMBER_INTEGER, "42"));
     * DataType type = expr->getType(); // DataType::INTEGER
     * @endcode
     */
    virtual Data::DataType getType() const = 0;
};
