...
namespace Sad
{
    namespace AST
    {
        ...
        public:
            ...
            // (AR) مُعدِّلات التصريح — «الصفة بعد الموصوف»: «متغير ساكن س»، «متغير عام س».
            //      قبل ISSUE-120 لم تُحلَّل البتّة، فكان المُعدِّل يُقرأ **اسمًا للمتغيّر**:
            //      «متغير ثابت ص = 10» يُصرِّح «ثابت» ثمّ يُسنِد إلى «ص» إسنادًا حرًّا —
            //      وذلك أصلُ ISSUE-030 (الثبات لا يمنع) وISSUE-031 (تركيبٌ يفشل بـSEM001).
            // (AR) ⚠️ ISSUE-127 — الحقلان أدناه **مُخزَّنان ولا يقرؤهما محرّكٌ ولا مولِّد**:
            //      تسجيلٌ لما نصّت عليه الصيغةُ لا وعدٌ بدلالة. فلا يُبنى عليهما حكمٌ
            //      («هذا ساكنٌ فعلًا»، «هذا خاصٌّ فلا يُرى») قبل أن يوصَلا بقارئ.
            //      مدّةُ التخزينِ الساكنةُ لغير العوامّ غيرُ منفَّذةٍ أصلًا (SEM039).
            // (EN) ISSUE-127 — stored, never consumed. Do not infer semantics from them.
            bool isStatic = false;           ///< (AR) «ساكن» — مسجَّلٌ لا مستهلَك (ISSUE-127)
            AccessModifier access = AccessModifier::PUBLIC; ///< (AR) مسجَّلٌ لا مستهلَك (ISSUE-127)
