...
namespace Sad
{
    namespace Security
    {
        namespace Taint
        {
            ...
            {
                for (const auto &mark : other.marks_)
                    marks_.push_back(mark);
                if (!other.marks_.empty())
                    tainted_ = true;
            }
            ...
            struct TaintTracker::Impl
            {
                ...
                static std::string sourceDescription(SourceType source)
                {
                    ...
                    case SourceType::UserInput:
                        return "إدخال المستخدم";
                    case SourceType::Network:
                        return "بيانات الشبكة";
                    case SourceType::File:
                        return "بيانات ملف";
                    case SourceType::Database:
                        return "بيانات قاعدة البيانات";
                ...
                static SinkType functionSinkType(const std::string &fn)
                {
                    ...
                    if (fn.find("HTML") != std::string::npos)
                    ...
                    if (fn.find("ملف") != std::string::npos)
                        return SinkType::FilePath;
                    return SinkType::Html;
                }
                ...
                static std::string warningMessage(SinkType sink)
                {
                    ...
                    {
                    case SinkType::SqlQuery:
                        return "SQL Injection: tainted data used in SQL query";
                    case SinkType::Html:
                        return "XSS: tainted data rendered into HTML";
                    case SinkType::SystemCmd:
                        return "Command Injection: tainted data passed to system command";
                    case SinkType::FilePath:
                        return "Path Traversal: tainted data used as file path";
                    default:
                ...
                static std::string fixSuggestion(SinkType sink)
                {
                    ...
                    case SinkType::Html:
                        return "Escape with sanitizeHTML() before rendering";
                    case SinkType::SystemCmd:
                        return "Avoid system commands or use a strict allowlist";
                    case SinkType::FilePath:
                        return "Use sanitizePath() and isPathWithinDirectory()";
                    default:
                        return "Sanitize data before use";
                    }
                }
                ...
                static bool sanitizes(const std::string &method, SinkType forSink)
                {
                    ...
                    {
                        if (r.sink == forSink && method == r.method)
                            return true;
                    }
                    return false;
                }
...
extern "C"
{
    ...
    {
        if (tracker)
            delete reinterpret_cast<TaintTracker *>(tracker);
    }
