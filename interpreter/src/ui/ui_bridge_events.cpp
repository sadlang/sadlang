...
namespace Sad
{
    namespace Interpreter
    {
        ...
        // ═══════════════════════════════════════════════════════════════════════════════
        // إعادة بناء الواجهة (Reactive Rebuild)
        // ═══════════════════════════════════════════════════════════════════════════════
        ...
        void UIBridge::rebuildUI()
        {
            ...
            // مسح الـ handlers القديمة (ستُعاد تسجيلها عند التحويل)
            ...
            // إعادة تحويل شجرة الـ widget إلى IR
        ...
        // ═══════════════════════════════════════════════════════════════════════════════
        // تسجيل واستدعاء handlers الأحداث
        // ═══════════════════════════════════════════════════════════════════════════════

        // ═══════════════════════════════════════════════════════════════════════════════
        // Tree Diffing — مقارنة شجرتين IR لاكتشاف التغييرات البصرية
        // ═══════════════════════════════════════════════════════════════════════════════

        bool UIBridge::treesEqual(const std::shared_ptr<sad::ui::IRNode> &a,
                                  const std::shared_ptr<sad::ui::IRNode> &b)
        {
            // كلتاهما فارغتان — متساويتان
            if (!a && !b)
                return true;
            // إحداهما فارغة — مختلفتان
            if (!a || !b)
                return false;
            ...
            // مقارنة الخصائص البصرية (نص، لون، أبعاد...)
            ...
            // مقارنة الأحداث (معرّفات الـ handlers حتمية لأن العدّاد يُصفّر)
        ...
        void UIBridge::updateTimers()
        {
            ...
            for (auto &t : timers_)
            {
                if (!t.cancelled && now >= t.fireTimeMs)
                {
                    ...
                    {
                        t.cancelled = true; // مرة واحدة: إلغاء بعد التنفيذ
                    }
                }
            }
            ...
            // إعادة رسم إن كان هناك مؤقتات نُفذت
            if (!toFire.empty())
            {
                rebuildUI();
            }
