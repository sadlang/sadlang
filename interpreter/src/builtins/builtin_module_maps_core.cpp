    ...
    namespace Interpreter
    {
        ...
        void registerBuiltinsMapsCore(Interpreter &interpreter)
        {
            ...
            auto map_constructor_fn = [](Sad::Interpreter::BuiltinContext &ctx) -> std::shared_ptr<Data::Value>
            {
                ...
                // (AR) 🔑 ولا تُسَدَّ هذه: `خريطة()` **بانيةٌ رتبتُها الدنيا صفر**،
                //      والخريطةُ الفارغةُ جوابُها الصحيحُ لا قيمةٌ زائفةٌ تُخترَع.
                //      سُدَّت مرّةً بالخطأ فأحمرَّت تسعَ بذَراتٍ سلوكيّةً بـRUN037 —
                //      والفرقُ أنّ الزائفَ **يخترعُ** جوابًا لِما لا جوابَ له،
                //      والافتراضيَّ **يُعطي** الجوابَ الذي تعنيه اللغة.
                if (args.empty())
                {
