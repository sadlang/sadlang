...
namespace Sad
{
    namespace Interpreter
    {
        ...
        void StatementExecutor::visitTryStmt(AST::TryStmt &node)
        {
            ...
            {
                // (AR) حفظ حالة التحكم بالتدفق قبل finally — وتصفير مؤقت
                //      حتى تُنفَّذ جميع جمل finally كاملةً دون قطع (حتى لو كان RETURN نشطاً)
                // (EN) Save and temporarily clear flow control before finally —
                //      so all finally statements execute fully without early exit (even with active RETURN)
                auto savedFlowBeforeFinally = flowControl_;
                Data::Value savedReturnValue = returnValue_;
                flowControl_ = FlowControl::NONE; // (AR) تعليق RETURN مؤقتاً ليكتمل finally
                ...
                {
                    // (AR) finally انتهى بدون return — استعادة حالة التحكم السابقة
                    // (EN) finally ended without return — restore previous flow control state
                    flowControl_ = savedFlowBeforeFinally;
                    returnValue_ = savedReturnValue;
                    // (AR) إذا كانت الاستعادة RETURN، يُلغي الاستثناء لصالح الإرجاع
                    if (flowControl_ == FlowControl::RETURN)
                    {
                        pendingException = nullptr;
                    }
        ...
        void StatementExecutor::visitMatchStmt(AST::MatchStmt &node)
        {
            ...
            for (auto &caseClause : node.cases)
            {
                ...
                else if (orPat && testValue.getKind() != Types::SadTypeKind::Map)
                {
                    // ═══════════════════════════════════════════════════════════════
                    // (AR) إصلاح X07: OR pattern مع تعدادات بسيطة
                    //      المشكلة: OrPattern::matches() يستدعي EnumVariantPattern::matches()
                    //      التي تتوقع MAP لكن القيمة INTEGER، فتُرجع false دائماً
                    //      الحل: نُطبق منطق المقارنة المباشرة على كل بديل في OR
                    // (EN) FIX X07: OR pattern with simple enums
                    //      Problem: OrPattern::matches() calls EnumVariantPattern::matches()
                    //      which expects MAP but value is INTEGER, always returning false
                    //      Fix: Apply direct comparison logic to each alternative in OR
                    // ═══════════════════════════════════════════════════════════════
                    for (const auto &alt : orPat->alternatives)
                    {
                        auto *altEnumPat = dynamic_cast<AST::EnumVariantPattern *>(alt.get());
                        if (altEnumPat)
                        {
                            // (AR) مقارنة مباشرة مع قيمة "تعداد.عضو"
                            // (EN) Direct comparison with "Enum.Member" value
                            std::string qualifiedName = altEnumPat->enumName + "." + altEnumPat->variantName;
                            const Data::Value *enumMemberVal = variableManager_.tryGet(qualifiedName);
                            if (enumMemberVal && (testValue == *enumMemberVal).toBool())
                            {
                                matched = true;
                                break;
                            }
                        ...
                        {
                            // (AR) بديل غير enum — استخدم matches() العادي
                            // (EN) Non-enum alternative — use normal matches()
                            std::map<std::string, Data::Value> temp_bindings;
                            if (alt->matches(testValue, temp_bindings))
                            {
