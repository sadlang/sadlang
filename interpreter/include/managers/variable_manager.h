...
namespace Sad
{
    namespace Data
    {
        ...
        public:
            ...
            // DEBUG helper
            Scope *debugGetCurrentScope() const { return scopeManager_.getCurrentScope(); }
            ...
            /**
             * @brief (AR) التقاط لقطة من جميع المتغيرات المرئية في سلسلة النطاقات الحالية
             * @brief (EN) Capture a snapshot of all visible variables in current scope chain
             *
             * (AR) تُستخدم لإنشاء نسخة من المتغيرات المرئية لتمريرها إلى goroutine مستقل.
             *      القيم تُنسخ (OBJECT يُشارك shared_ptr — وهو المطلوب للقنوات).
             * (EN) Used to capture visible variables for passing to an independent goroutine.
             *      Values are copied (OBJECT shares shared_ptr — which is desired for channels).
             *
             * @return (AR) خريطة اسم المتغير → قيمته / (EN) Map of variable name → value
             */
