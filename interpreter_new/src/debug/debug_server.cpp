...
void DebugServer::readerLoop() {
    while (connected_ && !shouldDisconnect_) {
        ...
        if (line.empty() && shouldDisconnect_) break;
        if (!line.empty()) {
            processCommand(line);
        }
    }
...
std::vector<DebugStackFrame> DebugServer::collectStackFrames() {
    ...
    for (int i = static_cast<int>(callStack_.size()) - 1; i >= 0; i--) {
        ...
        if (i == static_cast<int>(callStack_.size()) - 1 && !callStack_.empty()) continue;
...
std::vector<DebugVariable> DebugServer::collectVariables(int scopeRef) {
    ...
    if (!varMgr_ || !scopeMgr_) return vars;
    ...
    if (scopeRef == 1) {
        ...
        for (const auto& name : names) {
            ...
            if (val) {
                ...
                if (val->getType() == Data::ValueType::ARRAY) {
                    dv.variablesReference = 100 + static_cast<int>(vars.size());
                } else if (val->getType() == Data::ValueType::MAP) {
                    dv.variablesReference = 200 + static_cast<int>(vars.size());
                } else if (val->getType() == Data::ValueType::OBJECT) {
...
std::string DebugServer::evaluateExpression(const std::string& expression) {
    if (!varMgr_) return "<لا يوجد مفسر متصل>";
...
void DebugServer::onBeforeStatement(const std::string& file, int line, int column) {
    if (!connected_ || shouldDisconnect_) return;
    (void)column;
