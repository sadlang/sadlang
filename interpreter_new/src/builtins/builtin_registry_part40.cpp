/**
 * @file builtin_registry_part40.cpp
 * @brief (AR) دوال المقابس عالية المستوى — TCP و UDP
 *        تستخدم WinSock2 على Windows و POSIX sockets على Linux/Mac
 *        مستقلة عن مكتبة sad_network — تعمل مباشرة مع واجهة نظام التشغيل
 *
 * @brief (EN) High-level socket functions — TCP and UDP
 *        Uses WinSock2 on Windows and POSIX sockets on Linux/Mac
 *        Self-contained, no dependency on sad_network library
 *
 * @details
 * (AR) الدوال المتوفرة:
 *
 *   ═══ TCP — بروتوكول تدفق موثوق ═══
 *   1.  مقبس_tcp_جديد()                → رقم (معرف المقبس)
 *   2.  مقبس_اتصال(معرف، عنوان، منفذ)  → منطقي
 *   3.  مقبس_ارسال(معرف، بيانات)        → رقم (عدد البايتات المرسلة)
 *   4.  مقبس_استقبال(معرف، حجم)         → نص (البيانات المستلمة)
 *   5.  مقبس_ربط(معرف، منفذ)           → منطقي
 *   6.  مقبس_استماع(معرف، طابور)        → منطقي
 *   7.  مقبس_قبول(معرف)                → رقم (معرف المقبس الجديد)
 *   8.  مقبس_اغلاق(معرف)               → منطقي
 *   9.  مقبس_متصل(معرف)                → منطقي
 *   10. مقبس_عنوان_بعيد(معرف)           → نص
 *
 *   ═══ UDP — بروتوكول رسائل بدون اتصال ═══
 *   11. مقبس_udp_جديد()                → رقم (معرف المقبس)
 *   12. مقبس_udp_ربط(معرف، منفذ)       → منطقي
 *   13. مقبس_udp_ارسال(معرف، بيانات، عنوان، منفذ) → رقم
 *   14. مقبس_udp_استقبال(معرف، حجم)    → خريطة {بيانات، عنوان، منفذ}
 *   15. مقبس_udp_اغلاق(معرف)           → منطقي
 *
 *   ═══ إعدادات عامة ═══
 *   16. مقبس_مهلة_استقبال(معرف، ملي_ثانية)  → منطقي
 *   17. مقبس_مهلة_ارسال(معرف، ملي_ثانية)    → منطقي
 *   18. مقبس_بدون_تأخير(معرف، تفعيل)        → منطقي
 */
...
void registerBuiltinsPart40(Interpreter& interpreter) {
