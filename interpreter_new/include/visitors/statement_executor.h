...
namespace Sad
{
    namespace Interpreter
    {
        ...
        public:
            ...
            /**
             * @brief (AR) تعيين قيمة الإرجاع مباشرة وتفعيل إشارة RETURN
             *        يُستخدم من ExpressionEvaluator عند تنفيذ 'انشر' (error propagation)
             *        لإجبار الدالة الحالية على الإرجاع المبكر
             * @brief (EN) Set return value directly and activate RETURN signal
             *        Used by ExpressionEvaluator for 'انشر' (error propagation)
             *        to force early return from current function
             */
            void setReturnValueDirect(const Data::Value &val)
            {
                returnValue_ = val;
            ...
            /**
             * @brief (AR) زيارة جملة التأجيل — تسجيل جملة للتنفيذ عند خروج الدالة
             * @brief (EN) Visit defer statement — register statement for execution on function exit
             */
            void visitDeferStmt(AST::DeferStmt &node) override;

            /**
             * @brief (AR) زيارة جملة أطلق — إطلاق goroutine (مهمة متزامنة خفيفة)
             * @brief (EN) Visit go statement — launch a goroutine (lightweight concurrent task)
             */
            void visitGoStmt(AST::GoStmt &node) override;

            /**
             * @brief (AR) زيارة حالة اختيار من قناة
             * @brief (EN) Visit select case
             */
            void visitSelectCase(AST::SelectCase &node) override;

            /**
             * @brief (AR) زيارة جملة اختر — اختيار من قنوات متعددة
             * @brief (EN) Visit select statement — multiplex on multiple channels
             */
            ...
            /**
             * @brief (AR) زيارة كتلة امتداد — إضافة دوال لنوع موجود / (EN) Visit extension block
             */
            void visitExtensionDecl(AST::ExtensionDecl &node) override;

            /**
             * @brief (AR) زيارة تصريح ماكرو — تخزين الماكرو للتوسيع لاحقاً / (EN) Visit macro declaration
             */
            void visitMacroDecl(AST::MacroDecl &node) override;

            /**
             * @brief (AR) زيارة تصريح اسم مستعار للنوع — تسجيل الاسم كمتغير يشير للنوع الهدف
             *        (EN) Visit type alias declaration — register alias name pointing to target type
             */
            void visitTypeAliasDecl(AST::TypeAliasDecl &node) override;

            /**
             * @brief (AR) تفكيك الصف — تعيين عناصر صف إلى متغيرات فردية
             *        (EN) Tuple destructuring — assign tuple elements to individual variables
             */
            void visitTupleDestructureStmt(AST::TupleDestructureStmt &node) override;
            ...
            /**
             * @brief (AR) زيارة جملة إعادة التصدير / (EN) Visit re-export statement
             * @details صدّر * من وحدة / صدّر عنصر1 من وحدة
             */
            ...
            // (AR) دوال زائر واجهة المستخدم / (EN) UI visitor methods
            void visitUIDeclaration(AST::UIDeclarationNode &node) override;
            ...
            /**
             * @brief (AR) البحث عن ماكرو بالاسم / (EN) Find macro by name
             * @param name (AR) اسم الماكرو / (EN) Macro name
             * @return (AR) مؤشر لعقدة الماكرو أو nullptr / (EN) Pointer to macro node or nullptr
             */
            AST::MacroDecl *findMacro(const std::string &name) const
            {
            ...
            /**
             * @brief (AR) تعيين نوع الإرجاع الموحد للدالة الحالية
             * @brief (EN) Set the unified return type for current function
             */
            void setCurrentSadReturnType(Types::SadTypePtr type) { currentFunctionSadReturnType_ = std::move(type); }

            /**
             * @brief (AR) الحصول على نوع الإرجاع الموحد للدالة الحالية
             * @brief (EN) Get the unified return type for current function
             */
            const Types::SadTypePtr &getCurrentSadReturnType() const { return currentFunctionSadReturnType_; }
        ...
        private:
            ...
            // =========================================================================
            // (AR) مكدس التأجيل (defer stack)
            // =========================================================================
            // (AR) كل دالة لها مكدس تأجيل خاص — الجمل المؤجلة تُنفَّذ بترتيب LIFO
            //      عند خروج الدالة (ارجع، انتهاء طبيعي، أو استثناء).
            //      نستخدم vector من vectors: كل مستوى دالة له مكدسه الخاص.
            // (EN) Each function has its own defer stack — deferred statements execute
            //      in LIFO order on function exit (return, normal end, or exception).
            //      We use vector of vectors: each function level has its own stack.
            ...
            Types::SadTypePtr currentFunctionSadReturnType_;
            ...
            // =========================================================================
            // (AR) تخزين الماكروز / (EN) Macro Storage
            // =========================================================================

            // (AR) خريطة الماكروز — تُخزن مؤشرات لعُقد AST (AST يملك الذاكرة)
            // (EN) Macros map — stores pointers to AST nodes (AST owns the memory)
            std::unordered_map<std::string, AST::MacroDecl *> macros_;
        ...
        public:
            ...
            AST::TemplateClassDecl *getTemplateClass(const std::string &name)
            {
                ...
                if (it != templateClasses_.end())
                    return it->second;
                return nullptr;
            }

            /**
             * @brief (AR) الحصول على دالة قالب بالاسم
             * @brief (EN) Get a template function by name
             */
            AST::TemplateFunctionDecl *getTemplateFunction(const std::string &name)
            {
                auto it = templateFunctions_.find(name);
            ...
            /**
             * @brief (AR) التحقق من قيود القالب عند الإنشاء
             * @brief (EN) Verify template constraints at instantiation time
             */
            void verifyTemplateConstraints(
            ...
            AST::TemplateClassDecl *getTemplateInstanceSource(const std::string &className)
            {
                ...
                if (it != templateInstanceSources_.end())
                    return it->second;
                return nullptr;
            }
        };
