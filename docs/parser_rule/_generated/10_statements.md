...
- **الوصف:** تفرّع شرطي؛ الكتلة قد تُغلَق بـ«نهاية» أو ضمنياً بـ«وإلا»
...
- **الوصف:** حلقة تكرار على مصفوفة/مدى/خريطة؛ تدعم التفكيك «لكل فهرس، عنصر في ...» (بلا أقواس)
...
#### 🧩 تفصيل البدائل
- `«لكل» «IDENTIFIER» [ «،» «IDENTIFIER» ] «في» expression block`
...
- **الوصف:** مطابقة أنماط بكلمة «طابق»؛ النقطتان «:» بعد النمط اختيارية. تفاصيل الأنماط في gr.pattern.*. ملاحظة: «حالة» (KEYWORD_CASE) بناء شقيق يوزّعه parseSwitchStmt (parser_main.cpp:1200) — يُغطَّى لاحقاً منفصلاً.

#### 📐 BNF
```bnf
MatchStatement = 'طابق' '(' Expression ')'
                 { 'عندما' Pattern [ ':' ] { Statement } }
                 [ 'افتراضي' [ ':' ] { Statement } ] 'نهاية' ;
```

#### 🧩 تفصيل البدائل
- `«طابق» «(» expression «)» ( «عندما» pattern [ «:» ] { statement } )+ [ «افتراضي» [ «:» ] { statement } ] «نهاية»`
...
- **الوصف:** معالجة الأخطاء؛ «امسك خطأ» بلا أقواس (خطأ = اسم المتغير الملتقَط)
...
- **الوصف:** بناء شقيق لـ«طابق» بكلمة «حالة» (KEYWORD_CASE)؛ الأقواس «(تعبير)» أُزيلت. «:» اختياريّة بعد القيمة و«افتراضي». «افتراضي» يجب أن يكون الأخير. «حالة» سياقيّة (تُستثنى مع ( . [ أو عوامل إسناد).
