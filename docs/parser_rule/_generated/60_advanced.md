...
#### 📐 BNF
```bnf
Type = TypeCore [ '؟' | ( 'عدمي' | 'عدمية' ) Identifier<lookahead, same-line> ] ; TypeCore = 'رقم' | 'عشري' | 'نص' | 'منطقي' | 'خالي' | 'عدم' | 'أي'
         | ( 'مصفوفة' | 'خريطة' ) [ '<' Type [ ',' Type ] '>' ] | Identifier ;
```

#### 🧩 تفصيل البدائل
- `( «رقم» | «عشري» | «نص» | «منطقي» | «خالي» | «مصفوفة» | «خريطة» | «IDENTIFIER» ) [ «<» type { ( «،» | «,» ) type } «>» ] [ ( «؟» | «عدمي» ) ]`
