...
- **الوصف:** اسم نوع يُلفَظ كـIDENTIFIER (الأنواع **غير محجوزة** في المعجم). الأنواع المدمجة شائعة الاستعمال: «رقم» (صحيح)، «عشري»، «نص»، «منطقي»، «مصفوفة»، «خريطة»، «أي»؛ ويصلح أيّ معرّف صنفٍ مُعرَّفٍ من المستخدم. يظهر في تصريح المتغير («متغير نص اسم»)، ونوع المعامل («دالة ت(رقم س)»)، ونوع إرجاع الدالة («دالة رقم جمع()»). لاحقة «؟» تجعل النوع اختياريّاً (أمان العدم، NS-06).
...
- **الوصف:** صيغتان: (أ) «استورد وحدة» لوحدة كاملة، مع اسم مستعار اختياريّ «استورد وحدة كـ ر»؛ (ب) «من وحدة استورد اسم [، اسم…]» أو «من وحدة استورد *» لأسماء محدَّدة. **كلتاهما تُحلَّلان نحوياً بنجاح** — فشل «من … استورد» في ISSUE-024 خطأُ **تنفيذٍ** (الرمز غير موجود في الوحدة/تحليل الوحدة) لا خطأ نحو؛ فالبناء سليم والعائق دلاليّ (توفّر الرمز في الوحدة المضمَّنة). بعض المدمجات تتطلّب «استورد أساسيات» أوّلاً. **⚠️ تباعُدٌ تامٌّ متعاكسٌ في الصيغة (أ) — ISSUE-090 (مقيسٌ حيًّا 2026-08-03):** اسمُ الوحدة (ومستعارُها «كـ ر») يُنشئ **فضاءَ أسماءٍ في المفسّر حصرًا**: `ر.جمع(1، 2)` تُرجع 3 بينما `جمع(1، 2)` المسطَّحةُ تُخطئ SEM004. والمصرّفُ **يُسطّح حصرًا**: `جمع(1، 2)` تُرجع 3 بينما `ر.جمع(1، 2)` تفشل بـ«Undefined variable 'ر'». فلا برنامجَ يستوردُ وحدةً كاملةً بالصيغة (أ) يعملُ في المحرّكَين معًا. الصيغةُ (ب) («من وحدة استورد اسم») سليمةٌ مزدوجًا وهي الصيغةُ الموصى بها حتى يُحسَم التباعُد.
...
- **الوصف:** إعادةُ التصدير تجعل وحدةً واجهةً لوحداتٍ أخرى، فيستورد المستعمِلُ منها رمزًا معرَّفًا في غيرها. ثلاثةُ أشكال: «صدّر *» المجرَّدة (بلا «من») ومعناها «صدّر كلَّ رموز هذه الوحدة العلنيّة» — وهي **لا-عمليّة** في المحرّكَين لأنّ رموزَ الوحدة متاحةٌ أصلًا، وتُستعمل توثيقًا للنيّة في وحدات المكتبة؛ و«صدّر * من وحدة» التي تُحمّل الوحدةَ المصدر؛ و«صدّر عنصر [كـ اسم] [، عنصر…] من وحدة» الانتقائيّة. **تباعُدُ محرِّكَين مقيسٌ حيًّا (ISSUE-089):** شكلا «من وحدة» يعملان في المحرّكَين معًا ما دام الاستعمالُ في الملفّ المُعيدِ للتصدير نفسِه — تُحمَّل الوحدةُ المصدر ويصير الرمزُ (أو اسمُه المستعار) قابلًا للنداء هناك. أمّا **العبورُ** — أي أن يستوردَ ملفٌّ ثالثٌ من الوحدةِ الواجهةِ رمزًا معرَّفًا في الوحدة المصدر — فيعمل في المفسّر وحدَه؛ والمصرّفُ يرفضه بـ«استدعاء دالة غير معرّفة» لأنّه لا يُعيد بثَّ الرمز عبر الوحدة الوسيطة. والاستيرادُ المباشرُ من الوحدة المصدر يُصرَّف سليمًا، فالعطبُ محصورٌ في العبور لا في مسار الوحدات. **فضُّ الالتباس مع gr.decl.export:** الشكلُ الانتقائيُّ يبدأ كما تبدأ «صدّر اسم» من قاعدة التصدير؛ يفضّهما المحلّلُ بنظرةِ رمزٍ واحدٍ بعد الاسم: إن كان «من» أو فاصلةً (عربيّةً أو إنجليزيّة) أو «كـ» فهي إعادةُ تصدير، وإلّا فهي تصديرُ تصريحٍ.
