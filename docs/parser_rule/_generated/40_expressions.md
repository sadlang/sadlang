...
المقارنةُ الترتيبيّةُ (`<` `<=` `>` `>=`) **لا-موقَّعةٌ حين يكون النوعُ الساكنُ لـ«أيٍّ» من المعامِلَين `طبيعي64`** — هيمنةٌ لا اشتراط. وهذا يُطابِق الحسابَ (`+` `−` `×`) الذي يهيمن فيه `طبيعي64` بمعامِلٍ واحد سلفًا. والمساواةُ (`==` `!=`) مساواةُ بتّاتٍ فلا تتأثّر بالإشارة.
