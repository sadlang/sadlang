...
>
> **ملاحظة على الكتلة العامّة:** `include_directories(... interpreter/include ...)` في الجذر تبقى تُظهِر ترويسات المفسّر لكلّ هدف (بما فيه المترجم) — سطحٌ أوسع من تصدير `sad_shared` الذي أُغلق هنا. نقلُها إلى `sad_interp PUBLIC` (وإزالتُها من الجذر) يمسّ رسمَ البناء كاملًا ⇒ يُؤجَّل إلى PR مكرَّس؛ يحرس سلامتَه اليومَ `check_interpreter_boundary.py` (المترجم لا يضمّ أيّ ترويسة مفسّر).
