...
> ⚠️ **سِجِلٌّ لعصرِ المحرّكَين — اقرأ هذا أوّلًا.**
>
> كُتِبَت هذه الوثيقةُ حينَ كان للّغةِ محرّكان. وقد **حُذِفَ المفسّرُ**
> الشجريُّ (`interpreter/`) وأداتُه `sad-run`، ومعهما `sad-repl` والآلةُ
> الافتراضيّة. فكلُّ ما فيها من `sad-run` و`sad_interp` و`sad_core`
> و`sad_runtime` و`sad_graphics_bridge` و`sad_profiler_lib` **لا وجودَ
> له في الشجرةِ اليوم**، وكلُّ جدولٍ فيه عمودٌ «يربطه المفسّر» يصفُ
> طرفًا زال.
>
> فالمسارُ اليومَ واحد: القراءةُ ← المعجميُّ ← النحويُّ ← AST ← الفحوصُ
> الدلاليّةُ ← SIR ← LLVM ← تنفيذيٌّ أصليّ. ولا تفرُّعَ بعدَ AST.
>
> والوثيقةُ باقيةٌ كما كُتِبَت لأنّها **سِجلُّ قرارٍ ومَعلَمُ حالةٍ**، ومحوُ
> ما قيلَ يومَ قيلَ يُفقِدُ السجلَّ قيمتَه. ومن أراد حالةَ اليومِ فليقرأ
> `scripts/codegen/check_layering.py` — فترويستُه مُقاسةٌ بشفرتِه.
...
flowchart TD
    ...
    subgraph BACK["run_backend — compiler_driver_backend.cpp:173"]
        ...
        B2["الإخراج حسب العلم:<br/>--أظهر-llvm (.ll) · --أظهر-bc (.bc)<br/>-S (.s) · -c (.o) · افتراضيًّا تنفيذيّ"]
...
- تعدّد الملفّات: عند الحاجة للربط يُترجَم كلّ مصدر إلى كائن ثمّ تُربَط
  الكائنات؛ صيغ الإخراج أحاديّة الوحدة (`--أظهر-llvm` وأخواتها) تُرفَض
...
sequenceDiagram
    ...
    U->>I: sad-run برنامج.ص [--جامع/--تعلم/--إنتاج]
...
- ولّد الوسيط وافحصه: `sad-build برنامج.ص --أظهر-llvm` (الكتلة الأولى، أنواع
