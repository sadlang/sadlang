...
> 2. **كتلةَ `تجميع … نهاية`** (أسمبليٌّ يكتبه المستعمل بيده). ملفُّ اللهجة
>    `language-truth/dialects/assembly.yaml` يُعلن `architectures: [i686]`، لكنّ
>    الإعلانَ **لا يُبَثُّ في الرأس المولَّد ولا يُفحَص عند الخفض**. مقيس
>    (٢٠٢٦‑٠٨‑١٢) بالمنمنمات **العربيّة** — واللاتينيّةُ تُردّ بـSEM025 فلا تصلح
>    شاهدًا: `تجميع` ⏎ `عطّل_المقاطعات` ⏎ `أوقف` ⏎ `نهاية` بـ
>    `--هدف=aarch64-unknown-elf` تخرج **بصفر** وتبثّ `"cli\n\thlt"` كما هو.
>    **دَينٌ مُعلَنٌ لم يُغلَق** — وهو أوسعُ بابٍ باقٍ في هذا
>    الصنف؛ إغلاقُه يستلزم توسيعَ مُولِّد اللهجة، ويغيّر ما يُقبَل من برامج،
>    فيستحقّ رقعةً مستقلّةً بقياسها.
