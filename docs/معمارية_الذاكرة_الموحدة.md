...
> **الإصدار:** 1.2  
> **الحالة:** المرحلة A مكتملة (A1+A2)، Phase E-3 توحيد المُرسِل مكتمل، B/C قيد التخطيط  
> **آخر تحديث:** Phase E-3 (إزالة `--dev`/`--hybrid` نهائياً + توحيد `dispatch()`)
> **المرجع التطبيقي:** [الأمان الموحَّد](الأمان_الموحد.md)

> ### ⚠ تحديث Phase E-3 (مهم)
> العَلَمان `--dev` و `--hybrid` (وما يكافئهما عربياً) **أُزيلا نهائياً** من
> المُفسِّر والمترجم وقاعدة التحليل. الأعلام الرسمية الآن **ثلاثة فقط**:
> - `--gc` — الافتراضي: جامع قمامة + ملكية معطَّلة (يحلّ محل `--dev` السابق).
> - `--learn` — تحذيرات + تعليم بدون توقف.
> - `--prod` — ملكية صارمة كاملة (تعطي خطأ قاتلاً).
>
> أيّ ذكر لـ `--dev` أو `--hybrid` في الأقسام التالية تاريخيّ ويوصف الحالة
> القديمة لأغراض المرجع. الكود الإنتاجي **يرفض** هذه الأعلام برسالة خطأ
> صريحة توجِّه المستخدم إلى البديل.
