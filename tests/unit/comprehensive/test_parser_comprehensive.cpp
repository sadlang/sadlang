...
/**
 * @brief (AR) التحقق الصارم من أن المحلل يُبلِّغ عن خطأ نحوي فعلاً
 *        — عبر hasErrors() أو استثناء. عكس parsesSuccessfully المتساهلة:
 *        هذه تُرجع true فقط إذا رُصد خطأ، فلا تنجح توتولوجياً (TEST-004 AC-04).
 * @brief (EN) Strict check that the parser actually REPORTS a syntax error
 *        — via hasErrors() or an exception. Unlike the lenient helpers,
 *        returns true only when an error is genuinely surfaced.
 */
static bool parseReportsError(const std::string& source) {
    try {
        Sad::Lexer::LexerCore lexer(source);
        Sad::Parser::ParserCore parser(lexer);
        auto program = parser.parseProgram();
        (void)program; // (AR) قد يُنتج عُقداً مع التعافي — المعيار هو hasErrors
...
int main() {
    ...
    // ══════════════════════════════════════════════════════════════════
    // مجموعة 20: أخطاء نحوية صارمة (TEST-004 AC-04) / Strict Syntax Errors
    // (AR) عكس مجموعة 16 المتساهلة: هنا نُثبت أن المحلل يُبلِّغ الخطأ فعلاً
    //      عبر hasErrors() — دفع منطق الرفض من E2E البطيء إلى الوحدة.
    // ══════════════════════════════════════════════════════════════════
    SAD_GROUP("Parser.StrictNegative / المحلل_النحوي.سلبي_صارم");

    SAD_TEST("NERR01: كلمة محجوزة كاسم متغير تُرفض", {
        // متغير دالة = 5 — «دالة» محجوزة فلا تصلح اسماً
        SAD_ASSERT_TRUE(parseReportsError(AR_VAR " " AR_FUNC " = 5"));
    });

    SAD_TEST("NERR02: كلمة محجوزة كاسم دالة تُرفض", {
        // دالة صنف() — «صنف» محجوزة
        SAD_ASSERT_TRUE(parseReportsError(AR_FUNC " " AR_CLASS "()"));
    });

    SAD_TEST("NERR03: قوس غير مغلق يُبلَّغ عنه", {
        // متغير x = (1 + 2 — تقوية SERR01 التوتولوجي
        SAD_ASSERT_TRUE(parseReportsError(AR_VAR " x = (1 + 2"));
    });

    SAD_TEST("NERR04: وإلا يتيمة بلا إذا", {
        SAD_ASSERT_TRUE(parseReportsError(AR_ELSE "\n" AR_PRINT "(1)"));
    });

    SAD_TEST("NERR05: إسناد بلا قيمة (تعبير ناقص)", {
        // متغير x = — نهاية الملف قبل التعبير
        SAD_ASSERT_TRUE(parseReportsError(AR_VAR " x = "));
    });
