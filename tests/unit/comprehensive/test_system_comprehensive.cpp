...
int main() {
    ...
    SAD_TEST("timestamp returns number", {
        ...
        SAD_ASSERT_TRUE(ts.getType() == ::Sad::Types::SadTypeKind::Integer ||
                        ts.getType() == ::Sad::Types::SadTypeKind::Float);
    });

    SAD_TEST("clock_time returns double", {
        auto result = SystemFunctions::clock_time();
        SAD_ASSERT_TRUE(result.getType() == ::Sad::Types::SadTypeKind::Float);
    });
    ...
    SAD_TEST("getEnv with PATH", {
        ...
        SAD_ASSERT_TRUE(result.getType() == ::Sad::Types::SadTypeKind::String);
    });
