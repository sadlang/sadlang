...
RAW="$TMP/raw.ص"
# (AR) 🔑 الهجاءُ «طول()» لا «الطول»: «الطول» طريقةُ **مصفوفةٍ** في
#      `type_methods.yaml`، والمُعلَنُ للنصِّ «طول» برتبةِ صفرٍ تُنادى بقوسَيها.
#      وترْكُ الهجاءِ القديمِ هنا يجعلُ السكربتَ لغمًا: المترجّمُ يرفضُه فتفشلُ
#      فحوصُه لسببٍ لا علاقةَ له بـNS-04.
#      ⚠️ ومتغيّرُ `RUN` أدناه يشيرُ إلى `sad-run.exe` — **المحرّكُ المحذوف** —
#      وهذا السكربتُ لا يُنادَى من أيِّ بوّابةٍ ولا سير عمل (مقيسٌ ٢٠٢٦-٠٩-٠٣).
#      فنصفُه ميّتٌ ونصفُه غيرُ مُشغَّل، وذلك دَينٌ مُدوَّنٌ لا مسكوتٌ عنه.
