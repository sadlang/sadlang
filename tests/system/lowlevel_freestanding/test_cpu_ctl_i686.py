"""
============================================================================
(AR) اختبارات منع انحدار للّبنة م-لغة.3.12: خفض مدمجات النظام إلى أسمبلي i686
     حرّ (freestanding) بدل نداءات runtime مستضافة sad_ll_* أو أسمبلي x86-64.
     - اقرأ_سجل_تحكم/اكتب_سجل_تحكم: كانت sad_ll_read_cr/write_cr (مستضاف) ⇒
       صارت `mov %crN` inline asm i686 حرًّا.
     - اقرأ_سجل_نموذج: كانت rdmsr+shlq/rax (x86-64) ⇒ صارت rdmsr + دمج edx:eax
       بلا تعليمات 64-بت.
     - ابطل_صفحة: invlpg بوسيط i64 ⇒ عنوان 32-بت حرًّا.
     - حرّاس ضدّ الإفراط: المستضاف يبقى على sad_ll_*/x86-64 بلا مسّ.
...
(EN) Regression tests for milestone 3.12: system builtins lower to freestanding
     i686 inline asm instead of hosted sad_ll_* calls / x86-64 asm.
----------------------------------------------------------------------------
