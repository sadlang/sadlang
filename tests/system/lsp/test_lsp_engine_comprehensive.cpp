...
// حارس انحدار: الرمز العلويّ المعرَّف مرّةً واحدةً يجب ألّا يُوسَم «تعريف مكرر» (ص-ت١٠٢).
// كان جمع الرموز يُشغّل خطة regex الاحتياطية *قبل* عبور AST فيجمع كل رمز علويّ مرّتين
// (regex + AST) ⇒ تحذير مكرّر كاذب على كل دالّة ومتغيّر في المستوى العلويّ. الإصلاح: نقل
// الخطة الاحتياطية إلى *بعد* العبور كي يعمل حارس already_exists كملء-فجوات لا كمصدر تكرار.
TEST(LspDiagnostics, تعريف_علويّ_وحيد_لا_يُوسَم_مكرّرًا) {
    EngineFixture fx;
    fx.open(u8"دالة رئيسية()\n"
            u8"    متغير س = 1\n"
            u8"    اطبع_سطر(س)\n"
            u8"نهاية\n"
            u8"متغير ص = 2\n");
    for (const auto& d : fx.last_diags)
        ASSERT_TRUE(d.code != "ص-ت١٠٢"); // لا تعريف مكرر لرمزٍ معرَّف مرّةً
}

// حارس مقابل: التعريف المكرّر *الحقيقيّ* يبقى مكشوفًا (الإصلاح ملء-فجوات لا تعطيل).
TEST(LspDiagnostics, تعريف_علويّ_مكرّر_حقيقيّ_يبقى_مكشوفًا) {
    EngineFixture fx;
    fx.open(u8"دالة حساب()\n"
            u8"نهاية\n"
            u8"دالة حساب()\n"
            u8"نهاية\n");
    int dup = 0;
    for (const auto& d : fx.last_diags)
        if (d.code == "ص-ت١٠٢") dup++;
    ASSERT_EQ(dup, 1); // تحذير واحد بالضبط للتعريف الثاني
}

// حارس [مراجعة Amelia مهمّ ١]: متغيّر محلّيّ يظلّل اسم دالّة علويّة يجب ألّا يُوسَم «تعريف مكرر».
// عبور AST لا ينزل إلى محليّات الأجسام، فتأتي «س» المحليّة من خطة regex الاحتياطية بعمق 0 خطأً؛
// وحارس already_exists لا يقمعها لاختلاف النوع (دالّة مقابل متغيّر). وسم is_fallback يستبعدها من
// كشف التكرار فيبقى معتمِدًا على معلومة نطاق AST وحدها ⇒ لا تحذير كاذب على التظليل عبر-النوعيّ.
TEST(LspDiagnostics, محلّيّ_يظلّل_دالّة_علويّة_لا_يُوسَم_مكرّرًا) {
    EngineFixture fx;
    fx.open(u8"دالة س()\n"
            u8"نهاية\n"
            u8"دالة ب()\n"
            ...
            u8"نهاية\n");
    for (const auto& d : fx.last_diags)
        ASSERT_TRUE(d.code != "ص-ت١٠٢"); // «س» الدالّة و«س» المحلّيّة ليستا تعريفًا مكرّرًا
}
