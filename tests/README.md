...
- `@posix_only`: تخطٍّ على منصّة غير POSIX. لبدائيّاتٍ لا نظير لها على ويندوز
  (`عمليات`). التخطّي يبقى في المقام فلا يرفع النسبة كذبًا — وليس بديلًا عن
  `@skip_compiler` ولا ذريعةً لإخفاء فشلٍ حقيقيّ على لينكس.
- `@arch: <قائمة>`: بوّابة **عتاد** لا نظام (i686 · x86_64 · aarch64، والاسم
  العائليّ `x86` يعني i686+x86_64). للاختبارات التي تقيس تعليمة عتاد بعينها في
  كتلة «تجميع». 🔑 والوسم **دَينٌ لا رخصة**: ما هو محمول يبقى بلا وسم فيُقاس على
  المنصّات كلّها، و`scripts/ci/check_asm_dialect_arch.py` يحاكم الاتّجاهين على
  معجم مصدر الحقيقة — وسمٌ أضيق يُخفي تغطيةً، وأوسعُ يُحمِّر منصّةً لا يصلح لها.
