...
namespace Sad
{
    namespace LowLevel
    {
        ...
        int NetworkManager::setSubnet(int nicId, const std::string &mask)
        {
            if (nicId < 0 || nicId >= Sad::Security::SafeArithmetic::assertSafeCast<int>(nics_.size(), "network_stack_size"))
                return -1;
            if (!parseIP(mask, nics_[nicId].subnet))
                return -1;
            return 0;
        }
        ...
        int NetworkManager::setGateway(int nicId, const std::string &gw)
        {
            if (nicId < 0 || nicId >= Sad::Security::SafeArithmetic::assertSafeCast<int>(nics_.size(), "network_stack_size"))
                return -1;
            if (!parseIP(gw, nics_[nicId].gateway))
                return -1;
            return 0;
        }
        ...
        int NetworkManager::ping(int nicId, const std::string &targetIP)
        {
            if (nicId < 0 || nicId >= Sad::Security::SafeArithmetic::assertSafeCast<int>(nics_.size(), "network_stack_size"))
                return -1;
            if (nics_[nicId].nicState < NICState::INITIALIZED)
                return -1;
                                    ...
                                    int srcPort, const std::string &data)
        {
            ...
            stats_.udpSent++;
            nics_[nicId].txPackets++;
            nics_[nicId].txBytes += NetConstants::ETH_HEADER_SIZE + NetConstants::IPV4_HEADER_SIZE + NetConstants::UDP_HEADER_SIZE + data.size();
