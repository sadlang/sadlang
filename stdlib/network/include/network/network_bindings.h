...
extern "C"
{
    ...
    // الحصول على عنوان الطرف الآخر كنص مباشر
    // Get peer address as string
    const char *sad_tcp_socket_remote_addr(void *socket);

    // استقبال نص مباشرة من مقبس TCP
    // Receive string directly from TCP socket
    ...
    // استقبال نص مباشرة من مقبس UDP (يتجاهل عنوان المرسل)
    // Receive string directly from UDP socket (ignores sender address)
    ...
    // تعيين مهلة الاستقبال لمقبس TCP/UDP عبر الفئة الأساسية
    // Set receive timeout for TCP/UDP socket through the shared base class
    bool sad_socket_set_recv_timeout(void *socket, int milliseconds);

    // تعيين مهلة الإرسال لمقبس TCP/UDP عبر الفئة الأساسية
    // Set send timeout for TCP/UDP socket through the shared base class
    ...
    // الحصول على المنفذ المحلي لمقبس TCP/UDP بعد الربط
    // Get local port of a bound TCP/UDP socket
