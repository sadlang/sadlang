...
> عرف المستودع **ثلاث بنى YAML مختلفة للأخطاء، لا تتطابق أيٌّ منها** (بقيت منها اثنتان بعد
> تقاعد بنية sadinfo في 2026-07-03). هذا الملف يشرح لأي نظام توثيق كلٌّ منها، ومسار الهجرة
> الرسمي. اقرأه قبل لمس أي ملف أخطاء YAML حتى لا تخلط الأنظمة.
...
## البنية ٣ — كتالوج بيانات sadinfo (كيانات) — **متقاعد (2026-07-03)**

**أين (كان):** `data/_schemas/error.schema.json` (v1) + مجلد لكل كيان `errors/error_e001/_index.yaml`
— **حُذف `data/` كاملاً مع تقاعد sadinfo (2026-07-03)**؛ ما يلي وصف تاريخي لفهم البقايا في الوثائق.
...
- **الوظيفة:** **ليس** رسائل وقت تشغيل، بل **بيانات وصفية** للفهرسة والتصفّح (`E0001` مفتاح للأدوات،
  `owners`/`severity`، اسم عرض قابل للترجمة). كان يخدمه `tools/sadinfo/` (حُذفت الأداة والكتالوج معاً؛ `shared/sadinfo_core/` لم يوجد على القرص قطّ).
- **كان** الـ schema المرجعي لكتالوج sadinfo — حُذف مع fixtures اختباراته
  (`tests/system/sadinfo/`) في التقاعد نفسه.
...
| **V5 (الأخطاء، مُنجَز)** | المولّد يقرأ `language-truth/errors/` + مربوط CMake (EM-V5-1)؛ sadinfo كان إسقاطاً مُولَّداً (EM-V5-2) ثم تقاعد 2026-07-03؛ V4 محذوف (EM-V5-3). **متبقٍّ: تجميع المُولَّد في C++ (EM-3)** |
...
- **٣ تقاعد:** كتالوج sadinfo كان إسقاطاً مُولَّداً من `language-truth/` (EM-V5-2)، ثم حُذف مع
  الأداة (2026-07-03) — لم يعد له وجود على القرص.
...
- **`language-truth/` (V5) هو المصدر الموحَّد المطلق**؛ كل أداة (اللغة، الموقع، LSP، المنسّق،
  الحزم) تستهلك **إسقاطاً مُولَّداً** منه — لا مصدراً يدوياً خاصاً بها.
- **محسوم (2026-06-09، ق-فرعي-2):** sadinfo كان **إسقاطاً مُولَّداً** من `language-truth/` لا مصدراً —
  اعتماده مصدراً = قلب التبعية. (تقاعدت الأداة وإسقاطها في 2026-07-03.)
- البنية ١ = المصدر النشط (V4 حُذف بإغلاق M2-004)؛ البنية ٢ = الـ schema المعتمَد؛ البنية ٣ = متقاعدة.
...
3. **لا تخلط الأنظمة:** أي بقايا بنمط sadinfo (`E####`/`owners`) في وثائق قديمة = كتالوج
   الأداة المتقاعدة، لا مصدر رسائل (حُذف `data/errors/` مع الأداة — 2026-07-03).
