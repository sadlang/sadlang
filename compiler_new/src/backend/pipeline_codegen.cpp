...
private:
    /**
     * دمج عمليات map المتتالية | Fuse consecutive map operations
     * 
     * حوّل(f) |> حوّل(g) → حوّل(x => g(f(x)))
     */
    void fuseMapOperations(PipelineChain& chain) {
        std::vector<PipelineOperation> optimized;
        
        for (size_t i = 0; i < chain.operations.size(); ++i) {
            if (chain.operations[i].functionName == "حوّل" &&
                i + 1 < chain.operations.size() &&
                chain.operations[i + 1].functionName == "حوّل") {
                // دمج العمليتين
    ...
    /**
     * تقديم عمليات الترشيح | Move filter operations early
     * 
     * حوّل(f) |> صفّي(p) → صفّي(p∘f⁻¹) |> حوّل(f)  // إذا أمكن
     * أو على الأقل نحافظ على ترتيب filter قبل map عندما يكون ممكناً
     */
    void moveFiltersEarly(PipelineChain& chain) {
        // تنفيذ مبسط: لا نغير الترتيب إذا كان filter يعتمد على نتيجة map
        // في التنفيذ الفعلي، نحتاج تحليل أعمق
    }
    
    /**
     * إزالة العمليات الميتة | Eliminate dead operations
     * 
     * خذ(0) → []
     * صفّي(خطأ_دائماً) → []
     */
    void eliminateDeadOperations(PipelineChain& chain) {
        ...
        for (const auto& op : chain.operations) {
            // تحقق من العمليات التي لا تفعل شيئاً
            if (op.functionName == "حوّل" && 
                op.lambdaBody == op.lambdaParams[0]) {
                // حوّل(|س| س) - دالة الهوية، نتخطاها
                continue;
            }
            
            if (op.functionName == "صفّي" && 
                op.lambdaBody == "صحيح") {
                // صفّي(|س| صحيح) - لا يرشح شيئاً، نتخطاها
                continue;
            }
    ...
    /**
     * اكتشاف فرص التقييم الكسول | Detect lazy evaluation opportunities
     */
    void detectLazyOpportunities(PipelineChain& chain) {
        // إذا كان المصدر لانهائي أو كبير جداً
        bool hasInfiniteSource = (chain.sourceType.find("مدى_لانهائي") != std::string::npos);
        
        // إذا كانت هناك عملية take
        bool hasTake = false;
        for (const auto& op : chain.operations) {
            if (op.functionName == "خذ" || op.functionName == "خذ_طالما") {
                hasTake = true;
                break;
            }
        }
        
        // تفعيل التقييم الكسول إذا كان المصدر لانهائي وهناك حد
        chain.isLazy = hasInfiniteSource || 
                       (chain.operations.size() > 5 && hasTake);
    }
    
    /**
     * تركيب lambdas | Compose lambdas
     */
...
// ═══════════════════════════════════════════════════════════════════════════════
//                      توليد SIMD للأنبوب | SIMD Pipeline Codegen
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * 🚀 مولد_SIMD | SIMDCodegen
 * 
 * يولد كود SIMD لعمليات الأنبوب المتوازية
 */
class SIMDCodegen {
public:
    /**
     * توليد كود SIMD لـ map | Generate SIMD code for map
     */
    std::string generateSIMDMap(
        ...
        ss << "; توليد SIMD لعملية حوّل\n";
        ss << "; المصفوفة: " << arrayPtr << "\n";
        ss << "; الطول: " << arrayLen << "\n";
        
        // تحديد عرض SIMD
        int simdWidth = getSIMDWidth(elementType);
        
        ss << "  ; عرض SIMD: " << simdWidth << "\n";
        ...
        ss << "remainder_loop:\n";
        ss << "  ; معالجة العناصر المتبقية واحداً تلو الآخر\n";
        ss << "  ; ...\n";
...
private:
    /**
     * تحديد عرض SIMD حسب النوع | Determine SIMD width by type
     */
...
/**
 * 🔌 واجهة_الأنبوب | PipelineInterface
 * 
 * الواجهة الرئيسية لاستخدام مولد كود الأنبوب
 */
...
public:
    /**
     * معالجة وتوليد كود لسلسلة أنبوب | Process and generate code for pipeline chain
     */
    ...
    /**
     * إنشاء سلسلة من تعبير AST | Create chain from AST expression
     */
    ...
    void addOperation(PipelineChain& chain, PipelineOperation op) {
        if (!chain.operations.empty()) {
            // تحديث نوع الإدخال من نوع الإخراج السابق
...
void testLazyPipeline() {
    ...
    std::cout << "\n═══ اختبار سلسلة أنبوب كسولة ═══\n";
    
    PipelineInterface iface;
    auto chain = iface.createChain("%infinite_range", "مدى_لانهائي");
    chain.isLazy = true;
    
    PipelineOperation filter_op;
    filter_op.type = PipelineOperationType::LAZY_OPERATION;
    filter_op.functionName = "صفّي";
