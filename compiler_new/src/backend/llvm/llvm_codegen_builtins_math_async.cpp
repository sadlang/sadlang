...
namespace Sad
{
    namespace LLVM
    {
        ...
        llvm::Value *LLVMCodeGen::emitBuiltinLog(std::shared_ptr<SIRInstruction> inst)
        {
            if (!inst || inst->operands.empty())
                return nullptr;
            llvm::Value *arg = resolveOperand(inst->operands[0]);
            ...
            llvm::Value *dArg = arg->getType()->isDoubleTy() ? arg : builder_->CreateSIToFP(arg, llvm::Type::getDoubleTy(*context_));
            llvm::FunctionType *ft = llvm::FunctionType::get(llvm::Type::getDoubleTy(*context_), {llvm::Type::getDoubleTy(*context_)}, false);
            llvm::FunctionCallee fn = module_->getOrInsertFunction("log", ft);
            llvm::Value *result = builder_->CreateCall(fn, {dArg}, "log.ret");
            if (inst->result.has_value())
                context_info_.namedValues[inst->result->name] = result;
            return result;
        }
        ...
        llvm::Value *LLVMCodeGen::emitBuiltinPow(std::shared_ptr<SIRInstruction> inst)
        {
            ...
            //      بدون هذا التحويل، bit pattern الـ double يُفسَّر كعدد صحيح خاطئ
            // (EN) Convert result from double to i64 since Sad treats numbers as i64
            //      Without this, the double's bit pattern gets misinterpreted as wrong integer
            result = builder_->CreateFPToSI(result, getInt64Type(), "pow.i64");
            if (inst->result.has_value())
                context_info_.namedValues[inst->result->name] = result;
            return result;
        }
