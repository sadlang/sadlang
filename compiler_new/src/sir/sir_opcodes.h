...
namespace sad::sir
{
    ...
    enum class Opcode
    {
        ...
        Mul,      // ضرب: %r = Mul(%a, %b)
        Div,      // قسمة: %r = Div(%a, %b)
        FloorDiv, // قسمة صحيحة أرضية: %r = FloorDiv(%a, %b)
        Mod,      // باقي: %r = Mod(%a, %b)
        Neg,      // سالب: %r = Neg(%v)
        ...
        UICreateWidget, // إنشاء عنصر: %w = UICreateWidget("نوع", [خصائص])
        UISetProperty,  // تعيين خاصية: UISetProperty(%w, "اسم", %قيمة)
        UIGetProperty,  // قراءة خاصية: %v = UIGetProperty(%w, "اسم")
        UIAddChild,     // إضافة ابن: UIAddChild(%أب, %ابن)
        UIRemoveChild,  // إزالة ابن: UIRemoveChild(%أب, %ابن)

        // --- الأحداث ---
        UIBindEvent, // ربط حدث: UIBindEvent(%w, "حدث", %معالج)
        UIEmitEvent, // إطلاق حدث: UIEmitEvent(%w, "حدث", %بيانات)

        // --- التخطيط والعرض ---
        UISetLayout, // تعيين تخطيط: UISetLayout(%w, "نوع", [خيارات])
        UIRender,    // عرض عنصر: UIRender(%جذر)
        UIRunApp     // تشغيل التطبيق: UIRunApp(%جذر, "عنوان")
    };
    ...
    public:
         ...
         * 1. تخصيص (Alloc)
         */
        static SirInstruction Alloc(ValueId result, SadTypePtr type)
        {
            SirInstruction inst;
        ...
        static SirInstruction Take(ValueId result, ValueId container, int index)
        {
            ...
            inst.operands.push_back(ConstantOperand{static_cast<int64_t>(index), sirInt32()});
        ...
        // ═══════════════════════════════════════════════════════════════
        // دوال إنشاء عمليات الواجهة الرسومية
        // ═══════════════════════════════════════════════════════════════

        /**
         * إنشاء عنصر واجهة: %w = UICreateWidget("زر", [خصائص])
         */
        static SirInstruction UICreateWidget(ValueId result,
                                             const std::string &widgetType,
                                             const std::vector<ValueId> &props = {})
        {
            SirInstruction inst;
            inst.opcode = Opcode::UICreateWidget;
            inst.result = result;
            inst.operands.push_back(ConstantOperand{widgetType, nullptr});
            for (auto &p : props)
            {
                inst.operands.push_back(ValueOperand{p});
            }
            inst.comment = "إنشاء عنصر: " + widgetType;
            return inst;
        }

        /**
         * تعيين خاصية: UISetProperty(%w, "لون", %قيمة)
         */
        static SirInstruction UISetProperty(ValueId widget,
                                            const std::string &propName,
                                            ValueId value)
        {
            SirInstruction inst;
            inst.opcode = Opcode::UISetProperty;
            inst.operands.push_back(ValueOperand{widget});
            inst.operands.push_back(ConstantOperand{propName, nullptr});
            inst.operands.push_back(ValueOperand{value});
            inst.comment = "تعيين خاصية: " + propName;
            return inst;
        }

        /**
         * قراءة خاصية: %v = UIGetProperty(%w, "عرض")
         */
        static SirInstruction UIGetProperty(ValueId result,
                                            ValueId widget,
                                            const std::string &propName)
        {
            SirInstruction inst;
            inst.opcode = Opcode::UIGetProperty;
            inst.result = result;
            inst.operands.push_back(ValueOperand{widget});
            inst.operands.push_back(ConstantOperand{propName, nullptr});
            inst.comment = "قراءة خاصية: " + propName;
            return inst;
        }

        /**
         * إضافة ابن: UIAddChild(%أب, %ابن)
         */
        static SirInstruction UIAddChild(ValueId parent, ValueId child)
        {
            SirInstruction inst;
            inst.opcode = Opcode::UIAddChild;
            inst.operands.push_back(ValueOperand{parent});
            inst.operands.push_back(ValueOperand{child});
            inst.comment = "إضافة ابن";
            return inst;
        }

        /**
         * ربط حدث: UIBindEvent(%w, "ضغط", %معالج)
         */
        static SirInstruction UIBindEvent(ValueId widget,
                                          const std::string &eventName,
                                          ValueId handler)
        {
            SirInstruction inst;
            inst.opcode = Opcode::UIBindEvent;
            inst.operands.push_back(ValueOperand{widget});
            inst.operands.push_back(ConstantOperand{eventName, nullptr});
            inst.operands.push_back(ValueOperand{handler});
            inst.comment = "ربط حدث: " + eventName;
            return inst;
        }

        /**
         * عرض عنصر: UIRender(%جذر)
         */
        static SirInstruction UIRender(ValueId root)
        {
            SirInstruction inst;
            inst.opcode = Opcode::UIRender;
            inst.operands.push_back(ValueOperand{root});
            inst.comment = "عرض الواجهة";
            return inst;
        }

        /**
         * تشغيل التطبيق: UIRunApp(%جذر, "عنوان")
         */
        static SirInstruction UIRunApp(ValueId root, const std::string &title)
        {
            SirInstruction inst;
            inst.opcode = Opcode::UIRunApp;
            inst.operands.push_back(ValueOperand{root});
            inst.operands.push_back(ConstantOperand{title, nullptr});
            inst.comment = "تشغيل التطبيق: " + title;
            return inst;
        }
    ...
    private:
        static std::string opcodeToString(Opcode op)
        {
            ...
            case Opcode::Unreachable:
                return "غير_قابل_للوصول";

            // عمليات الدوال
            case Opcode::Call:
                return "استدعاء";
            case Opcode::CallIndirect:
                return "استدعاء_غير_مباشر";
    ...
    public:
        ...
        SadTypePtr returnType; // (AR) نوع الإرجاع الموحد / (EN) Unified return type
        ...
        std::string toString() const
        {
            ...
            str += ")";
            if (returnType && returnType->getKind() != SadTypeKind::Void)
            {
    ...
    public:
        ...
        std::map<std::string, SadTypePtr> types; // (AR) أنواع الوحدة الموحدة / (EN) Module types (SadTypePtr)
