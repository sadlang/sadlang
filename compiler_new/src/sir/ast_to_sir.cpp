...
namespace sad::sir
{
    ...
    public:
        ...
        SirFunction lowerFunction(const std::shared_ptr<AstNode> &funcNode)
        {
            ...
            func.returnType = funcNode->type ? funcNode->type : sirVoid();
        ...
        void lowerVarDecl(const std::shared_ptr<AstNode> &varDecl, SirFunction &func)
        {
            ...
            auto type = varDecl->type ? varDecl->type : sirInt32();
    ...
    private:
        ...
        ValueId lowerIntLiteral(const std::shared_ptr<AstNode> &lit)
        {
            ...
            inst.result = id;
            inst.operands.push_back(TypeOperand{sirInt32()});
            emit(inst);
        ...
        {
            ValueId id = createValue();
            emit(SirInstruction::Alloc(id, sirFloat64()));
            return id;
        }
        ...
        {
            ValueId id = createValue();
            emit(SirInstruction::Alloc(id, sirString()));
            return id;
        }
        ...
        {
            ValueId id = createValue();
            emit(SirInstruction::Alloc(id, sirBool()));
            return id;
        }
        ...
        ValueId lowerBinaryOp(const std::shared_ptr<AstNode> &binOp, SirFunction &func)
        {
            ...
            else if (op == "//" || op == "قسمة_صحيحة")
            {
                inst.opcode = Opcode::FloorDiv;
            }
            else if (op == "%" || op == "باقي")
            {
