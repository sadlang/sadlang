...
private:
    ...
    void printFunction(const SirFunction& func) {
        ...
        if (func.returnType && func.returnType->kind != TypeKind::Void) {
            output_ << " -> ";
    ...
    void printLineNumber() {
        if (opts_.showLineNumbers) {
            if (opts_.useColors) output_ << colors::GRAY;
            output_ << std::setw(4) << lineNum_++ << " │ ";
            if (opts_.useColors) output_ << colors::RESET;
        }
    }
    ...
    void keyword(const std::string& kw) {
        if (opts_.useColors) output_ << colors::MAGENTA;
        output_ << kw;
        if (opts_.useColors) output_ << colors::RESET;
    }
    
    void opcode(Opcode op) {
        if (opts_.useColors) output_ << colors::CYAN;
        output_ << opcodeToString(op);
        if (opts_.useColors) output_ << colors::RESET;
    }
    
    void value(ValueId id) {
        if (opts_.useColors) output_ << colors::YELLOW;
        output_ << "%" << id;
        if (opts_.useColors) output_ << colors::RESET;
    }
    
    void constant(const std::string& c) {
        if (opts_.useColors) output_ << colors::GREEN;
        output_ << c;
        if (opts_.useColors) output_ << colors::RESET;
    }
    
    void label(const std::string& lbl) {
        if (opts_.useColors) output_ << colors::BLUE;
        output_ << lbl;
        if (opts_.useColors) output_ << colors::RESET;
    }
    ...
    void typeName(const std::shared_ptr<SirType>& type) {
        if (opts_.useColors) output_ << colors::BLUE;
