...
namespace Sad
{
    namespace TypeSystem
    {
        ...
        public:
             ...
             */
            PatternChecker()
                : useArabicMessages_(true), debugMode_(false), currentMatchType_(nullptr)
            {
            }
        ...
        private:
                ...
                TypePtr expectedType)
            {
                ...
                switch (pattern.literal.getKind())
                {
                ...
                TypePtr expectedType)
            {
                ...
                if (expectedType && expectedType->getKind() != SadTypeKind::Array)
                {
                    // (AR) قد يكون Union يحتوي على Array
                    // (EN) Might be Union containing Array
                    if (expectedType->getKind() != SadTypeKind::Union)
                    {
                        return PatternCheckResult::failure(formatError(
            ...
            {
                if (!expected)
                    return true; // (AR) أي نوع مقبول / (EN) Any type is acceptable
                if (!actual)
                    return false;
                ...
                auto &registry = TypeRegistry::getInstance();
                if (expected->equals(registry.getAnyType().get()))
                    return true;

                // (AR) التحقق من Union
                // (EN) Check Union
                if (expected->getKind() == SadTypeKind::Union)
                {
                    ...
                    if (unionType)
                    {
                        ...
                        {
                            if (isTypeCompatible(actual, member))
                                return true;
                        }
                    }
                }
