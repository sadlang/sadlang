                ...
                else if (auto *orPat = dynamic_cast<const Sad::AST::OrPattern *>(pattern))
                {
                    ...
                    for (const auto &alt : orPat->alternatives)
                    {
                        if (auto *altLit = dynamic_cast<const Sad::AST::LiteralPattern *>(alt.get()))
                        {
                            ...
                            SadTypeKind altLitType = matchValueType;

                            const auto &altVal = altLit->literal;
                            if (altVal.isInteger())
                            {
                                altLitValue = std::to_string(altVal.toInt());
                                altLitType = SadTypeKind::Integer;
                            }
                            else if (altVal.getKind() == SadTypeKind::Float)
                            {
                                altLitType = SadTypeKind::Float;
                                altLitValue = std::to_string(altVal.toDouble());
                            }
                            else if (altVal.getKind() == SadTypeKind::Boolean)
                            {
                                altLitValue = altVal.toBool() ? "true" : "false";
                                altLitType = SadTypeKind::Boolean;
                            }
                            else if (altVal.getKind() == SadTypeKind::String)
                            {
                                altLitValue = altVal.toString();
                                altLitType = SadTypeKind::String;
                            }
                            else
                            {
                                altLitValue = altVal.toString();
                            ...
                            if (matchValueType == SadTypeKind::Integer && altLitType == SadTypeKind::Float)
                            {
                                altLitValue = std::to_string(static_cast<int64_t>(altVal.toDouble()));
                                altLitType = SadTypeKind::Integer;
                            }
                            else if (matchValueType == SadTypeKind::Float && altLitType == SadTypeKind::Integer)
                            {
                            ...
                            if (altLitType == SadTypeKind::String)
                            {
                                altConstOp = SIROperand::ConstantString(altLitValue);
                            }
                            else if (altLitType == SadTypeKind::Float)
                            {
                                altConstOp = SIROperand::ConstantF64(std::stod(altLitValue));
                            }
                            else if (altLitType == SadTypeKind::Boolean)
                            {
                            ...
                            SIROperand cmpAltResultOp = SIROperand::Register(cmpAltReg, SadTypeKind::Boolean);
                            ...
                            SIROperand prevAccOp = SIROperand::Register(accumReg, SadTypeKind::Boolean);
                            SIROperand cmpResOp = SIROperand::Register(cmpAltReg, SadTypeKind::Boolean);
                            SIROperand newAccResultOp = SIROperand::Register(newAccumReg, SadTypeKind::Boolean);
