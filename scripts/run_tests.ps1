...
foreach ($t in $tests) {
    ...
    # (AR) 🔑 **وصيغةُ النقطتَينِ تُقرَأُ كهجاءِ الحارسِ والعدّاء.** كان
    #      النمطُ `"# @expected (.+)"` بلا نقطتَينِ ولا مرساةٍ، فيُهمِلُ
    #      **٣٢٩٧ بذرةً ذاتَ عقدٍ من ٤٥٤٥ في `tests/behavior`** — وترويستُه
    #      تعُدُّ سببَينِ لضِيقِه وتُغفِلُ هذا الحاسم. والمقيسُ في الاتّجاهِ
    #      الآخر: صفرٌ — أي أنّه ضيّقٌ لا مخالف.
    #      ⚠️ **والموروثُ وجهُ الهجاءِ وحدَه، لا القارئُ كلُّه** — والدرسُ
    #         مُدوَّنٌ أنّ توريثَ وجهٍ واحدٍ يُخفي الانجرافَ في البقيّة. ووجهانِ
    #         باقيانِ **دَينًا مُسمًّى**: ① النافذة — هذا يمسحُ الملفَّ كلَّه
    #         والحارسُ والعدّاءُ ثلاثينَ سطرًا؛ ② `strip` الصدريّ — العدّاءُ
    #         يُطابِقُ بعدَ `line.strip()` فيقبلُ وسمًا مُزاحًا وهذا لا يقبلُه.
    #         ولا أثرَ حيًّا اليومَ: قِيسَ على ٤٥٤٥ بذرةً ⇒ صفرٌ في الاتّجاهَين.
...
# (AR) و«لم يُنفَّذْ» تُقاسُ بالمجموعِ لا بالناجح: `pass -eq 0` كانت تُعلِنُ
#      «لم يُنفَّذْ اختبارٌ واحد» على شوطٍ **نُفِّذَ كلُّه وفشل**، وتردُّ ٢
#      (عطبَ آلة) حيثُ الصوابُ ١ (إخفاق) — فالحكمُ الذي يبلغُ المُستدعيَ كاذب.
# (AR) و`skipped` عدَّادُ **تجاوزِ المهلة** لا تخطٍّ مقصود: ضمُّه إلى
#      مقياسِ «هل نُفِّذَ شيء» جعلَ شوطًا **عَلِقَ كلُّه** يُقرأُ كشوطٍ نجحَ
#      كلُّه (٠ إخفاق) ⇒ رمزُ صفر — وهو عينُ ما وُجِدَتِ المهلةُ لالتقاطِه.
if (($pass + $fail + $skipped) -eq 0) {
    Write-Host "X لم يُنفَّذْ اختبارٌ واحد — صفرُ اختبارٍ ليس نجاحًا."
    exit 2
}
if ($fail -gt 0 -or $skipped -gt 0) { exit 1 }
exit 0
