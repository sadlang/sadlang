...
def اجمع_المخالفات() -> list:
    ...
    for ملف in sorted(جذر_الاختبارات.rglob("*.ص")):
        # (AR) 🔑 `utf-8-sig` لا `utf-8`: بادئةُ BOM تسبقُ `#` فالسطرُ الأوّلُ
        #      لا يبدأُ بها بعدَ `strip()` (و`\ufeff` ليس فراغًا في بايثون)،
        #      فما في السطرِ الأوّلِ يُقرأُ **غيرَ موجود**. (٨٦ بذرةً ببادئةٍ
        #      في الشجرة، وقد قِيسَ أثرُ نظيرِها: أربعُ بذورٍ كانت تُشخَّصُ
        #      «بلا وسم @rule» وهي تحملُه.)
        # (EN) utf-8-sig, not utf-8: a BOM precedes '#', so a first-line tag
        #      reads as absent.
        نص = io.open(ملف, encoding="utf-8-sig", errors="replace").read()
