...
if (-not $SkipBuild -and -not $HtmlOnly) {
    ...
    if ($Verbose) {
        # (AR) sad-run زال مع المفسّر — هدفُ التغطيةِ صار المترجّم.
        & $CMAKE --build $BUILD_COV --target sad-build --config Debug
    }
    else {
        & $CMAKE --build $BUILD_COV --target sad-build --config Debug 2>&1 | Select-Object -Last 8
    }
