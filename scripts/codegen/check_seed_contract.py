...
# (AR) 🔑 **وهجاءُ الوسمِ السالبِ يُعلَنُ ههنا مرّةً ويُستوردُ.** كان مكتوبًا
#      باليدِ ثلاثَ مرّات: مرّتَينِ في هذا الملفِّ و**مرّةً في
#      `check_anchor_integrity.py`** — الأخيرةُ خارجَ كلِّ مرساةٍ تحرسُ النسخ
#      (لا تستوردُ الحارسَ ولا تذكرُ اسمَه)، فهي القارئُ الوحيدُ في المجلَّدِ
#      الذي لم يكنْ يراه شيء. والثلاثةُ تقرأُ وسمًا واحدًا وتُشتَقُّ منه الآن.
NEGATIVE_TAG = r"^#[ \t]*@expect(?:_compile)?_error[a-z_]*"
_NEGATIVE = re.compile(NEGATIVE_TAG, re.M)
# (AR) وشكلُ الرمزِ يُشتقُّ من عقدِ العدّاءِ (`tests/runner.py`) لا يُخترَعُ ههنا.
_NEG_CODE = re.compile(NEGATIVE_TAG + r":?[ \t]+"
