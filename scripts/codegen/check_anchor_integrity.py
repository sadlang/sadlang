...
_ANCHOR_FORMS = (
    ...
    # (AR) ⚠️ و`[ \t]+` لا `\s+`: في `re.M` يعبرُ `\s` **نهايةَ السطرِ**، فوسمٌ
    #      عارٍ من حمولةٍ (`# @expect_error` وحدَه، وهو مُودَعٌ في ٣٠٠ موضع) يبتلعُ
...
#
# (AR) 🔑 **وشكلُ الرمزِ يُشتقُّ من عقدِ العدّاءِ لا يُخترَعُ ههنا.**
#      `tests/runner.py` يشترطُ `^[A-Z]{2,4}[0-9]{3}$`، و`[A-Za-z]+[0-9]+` أوسعُ
#      منه — فيقرأُ الحارسُ رمزًا لا يعترفُ به مَن يُنفِّذُ العقد. وقِيسَ أنّ
#      التضييقَ **بلا خسارة**: ١٣٣ مرساةً و٤٠ رمزًا قبلَه وبعدَه سواءً، وكلُّ
#      الـ١٩١ رمزًا المُعلَنِ يُطابِقُ الشكلَ الضيّق.
_CODE_HEAD = re.compile(r"[ \t]*([A-Z]{2,4}[0-9]{3})(?![0-9])")
...
#      ههنا اتّباعٌ لعُرفٍ مُدوَّنٍ لا تسهيلٌ على الحارس: شجرةٌ لا تُشغَّلُ لا
#      يُطالَبُ مُعلَنُها بشاهدٍ منها، ولا يُحاكَمُ ما فيها بعقدٍ حيّ.
...
#      🔑 **وبالمسارِ النسبيِّ لا بالاسمِ القاعديّ.** الاسمُ القاعديُّ يسري على
#         العائلاتِ الخمسِ كلِّها، فملفٌّ بهذا الاسمِ في `builtins/` كان يُبتلَعُ
#         صامتًا وهو مصدرُ إعلانٍ حقيقيّ. (بُرهنَ بالحقن.) والدرسُ مُدوَّنٌ في
#         الملفِّ الشقيقِ نفسِه: «هويّةُ الحارسِ مسارُه لا اسمُه القاعديّ».
SKIP_SOURCES = ("backend/arch_specific_opcodes.yaml",)
...
# (AR) 🔑 **وخفضُ أرضيّةٍ لا يكون إلّا بحذفٍ مُعلَنٍ مُسمًّى** — لا لأنّ العدّادَ
#      هبط. خُفِضَت «رموزُ الأخطاء» من ١٩١ إلى **١٨٩** في 2026-09-07 بحذفِ
#      رمزَينِ بأعيانِهما من الكتالوج: `SEM040 SEM_VOID_NOT_A_VALUE_TYPE` و
#      `SEM045 SEM_VOID_ASSIGNED_TO_TYPED_SLOT`. وقد حُذف «فراغ» من اللغةِ وحلَّ
#      محلَّه «خالي» — نوعُ الوحدة، وهو نوعُ قيمةٍ كاملٌ تحملُه الخانة — فزالَ
#      **موضوعُ** القاعدةِ لا شدّتُها. ورقماهما لا يُشغَلانِ بغيرِهما (فجوةٌ
#      محجوزةٌ منصوصةٌ في `language-truth/errors/semantic.yaml`).
#      ⚠️ ولولا هذا الخفضُ المكتوبُ لبقيَ الحارسُ أحمرَ يقولُ «انكمشَ المُعلَن»
#      وهو صادقٌ: الانكماشُ عطبٌ حتّى يُسمَّى، فتسميتُه هي التي تجعلُه حذفًا.
# (EN) A floor is lowered only by a NAMED deletion, never because the counter
#      dropped: 191 → 189 on 2026-09-07 with SEM040 and SEM045, deleted together
#      with the «فراغ» type whose rule they policed. Their numbers are never reused.
FLOOR_DECLARED = {
    "النحو": 107,
    "رموزُ الأخطاء": 189,
    "المدمجات": 1205,
...
#         عينُ ما التأهيلُ له. والأوپكوداتُ بلا نطاقٍ في مصدرِ الحقيقة، وكان
#         ذوبانُها يُقرأُ ٣٥ — وهو عددُ طبقةِ القيدِ لا دَينًا؛ فباستثنائِها
#         صارَ صفرًا كأخواتِه (انظر `SKIP_SOURCES`).
CEILING_DISSOLVED = {
    "النحو": 0,
    "رموزُ الأخطاء": 0,
    "المدمجات": 0,
...
CEILING_UNWITNESSED = {
    ...
    #      ثمّ ١٥١ ← **١٥٠** في 2026-09-07 بحذفِ `SEM045` من الكتالوج: رمزٌ كان
    #      مُعلَنًا بلا شاهدٍ فزالَ هو ودَينُه معًا. ⚠️ ولا يُخلَطُ هذا بسدِّ دَين:
    #      البندُ لم يُشهَدْ له، بل حُذف — والسقفُ يُنزَّلُ كي لا يُعادَ ملؤُه صامتًا.
    "رموزُ الأخطاء": 150,
    "المدمجات": 1205,
    "أوپكوداتُ الخلفيّة": 780,
    "المكتبةُ القياسيّة": 193,
...
def _declared() -> tuple[dict[str, set[str]], dict[str, int]]:
    ...
    for fam, sub, key, idk, nsk, prefix in FAMILIES:
        ...
        for path in sorted(base.rglob("*.yaml")):
            if ("_schemas" in path.parts
                    or path.relative_to(LT).as_posix() in SKIP_SOURCES):
                continue
