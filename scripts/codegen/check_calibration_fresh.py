...
#      ⚠️ **وهو ٢٠ لا ١٩**: المقيسُ محلّيًّا ١٩ لأنّ الشجرةَ تحملُ سجلَّ جلسةٍ
#         أخرى (`check_ui_render_completeness.yaml`) مُدرَجًا غيرَ مُودَع؛
#         و**استنساخٌ نظيفٌ لهذا الفرعِ يرى ٢٠** — لأنّ إيداعَ هذه الدفعةِ
#         مُقيَّدُ المسارات (`git commit -- <مساراتي>`) فلا يبتلعُ سجلَّ الجار.
#         وشدُّه إلى المقيسِ محلّيًّا
#         يُوقِفُ CI على أوّلِ استنساخٍ — وهي عينُ العلّةِ الموصوفةِ أعلاه
#         مقلوبةً. يُنزَّلُ إلى ١٩ حين يُودِعُ الجارُ سجلَّه.
CEILING_UNCALIBRATED = 20
...
# (AR) سجلّاتٌ لا تُعلِنُ `targets_sha256` — دَينٌ نازلٌ لا يُرفَع. وصمتُ
#      الحقلِ ليس تغطية: مجسّاتُ صاحبِه قد ترسو في ملفٍّ ثالثٍ لا يحرسُه شيء.
#      🔑 **والسقفُ مقيسٌ على ما يُعلِنُه المستودعُ (HEAD)** لا على شجرةِ عملٍ
#         يتشاركُها اثنان — وهي عينُ العلّةِ المُصلَحةِ في
#         `CEILING_UNMEASURED_HARNESSES`.
#      ⚠️ **وهو ١ لا ٠ عمدًا**: سجلّاتُ هذه الجلسةِ الخمسةُ كلُّها تُعلِنُ
#         الحقلَ (المقيسُ ٠)، لكنّ `check_ui_render_completeness.yaml` — سجلٌّ
#         لجلسةٍ أخرى قائمٌ في الشجرةِ ولم يُودَعْ بعد — لا يُعلِنُه. وشدُّ
#         السقفِ إلى ٠ يجعلُ **إيداعَ الجارِ** أحمرَ في CI على عملٍ لا صلةَ له
#         بهذا. فيُترَكُ موضعٌ واحد، ويُنزَّلُ حين يُعلِنُ الجارُ حقلَه.
CEILING_UNFINGERPRINTED = 1
...
def main() -> int:
    ...
    for rec in records:
        ...
        # (AR) و`version` يُقرَأُ ولا يبقى حقلًا نائمًا: شكلُ السجلِّ تبدّلَ
        #      بدخولِ `targets_sha256`، فالنسخةُ ٣ فما فوقُ **تُوجِبُه**،
        #      وما دونَها يُعَدُّ في الدَّينِ المُعلَنِ أدناه.
        version = doc.get("version")
        if not isinstance(version, int) or version < 2:
            incomplete.append(f"{rel} — نسخةُ سجلٍّ غيرُ معروفة: {version!r}")
            continue
        targets = doc.get("targets_sha256")
        if version >= 3 and not isinstance(targets, dict):
            incomplete.append(f"{rel} — نسخةُ {version} تُوجِبُ targets_sha256")
            continue
