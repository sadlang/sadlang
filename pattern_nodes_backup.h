...
public:
    ...
    bool matches(const Data::Value& value, 
        ...
        if (value.getType() != Data::ValueType::NUMBER && 
...
public:
    ...
    bool matches(const Data::Value& value, 
        ...
        for (const auto& [fieldName, fieldPattern] : fields) {
            // (AR) الحصول على قيمة الحقل / (EN) Get field value
            auto fieldValue = value.getProperty(fieldName);
            if (!fieldValue.has_value()) {
                return false;  // (AR) الحقل غير موجود / (EN) Field not found
            }
            
            if (!fieldPattern->matches(fieldValue.value(), bindings)) {
                return false;
            }
        }
