            ...
            bool CSEPass::runOnFunction(SIR::SIRFunction *function)
            {
                ...
                for (size_t blockIdx = 0; blockIdx < blocks.size(); ++blockIdx)
                {
                    ...
                    for (size_t instIdx = 0; instIdx < instructions.size(); ++instIdx)
                    {
                        ...
                        // (AR) إبطال إدخالات الجدول عند STORE لمنع CSE الخاطئ:
                        // إذا كانت التعليمة STORE (تخزين قيمة في متغير)، يجب إبطال
                        // جميع التعابير المحفوظة التي تعتمد على المتغير الوجهة.
                        // السبب: بعد STORE %r, %var، قيمة %var تغيّرت،
                        // فأي تعبير مثل ADD %var, 1 محفوظ مسبقاً لم يعد صالحاً.
                        ...
                        if (inst.opcode == SIR::SIROpcode::STORE && inst.operands.size() >= 2)
                        {
                                ...
                                dest.type == SIR::SIROperandType::GLOBAL)
                            {
                                ...
                                // حذف جميع الإدخالات التي تحتوي على المتغير الوجهة كمعامل
