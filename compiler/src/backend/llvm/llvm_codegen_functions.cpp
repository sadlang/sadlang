...
namespace Sad
{
    namespace LLVM
    {
         ...
         */
        void LLVMCodeGen::setLTOMode(bool enable_full, bool enable_thin)
        {
            if (optimizer_)
            {
                optimizer_->setLTOMode(enable_full, enable_thin);
            }
            ...
            if (module_)
            {
                if (enable_thin)
                {
                    if (!module_->getModuleFlag("ThinLTO"))
                        module_->addModuleFlag(llvm::Module::Error, "ThinLTO", 1);
                    if (!module_->getModuleFlag("EnableSplitLTOUnit"))
