    ...
    namespace LLVM
    {
        ...
        llvm::Value *LLVMCodeGen::emitBuiltinPrint(std::shared_ptr<SIRInstruction> inst)
        {
            ...
            for (auto &op : inst->operands)
            {
                ...
                {
                    llvm::Value *fmt = builder_->CreateGlobalStringPtr("%s", "fmt.s");
                    llvm::Value *trueStr = builder_->CreateGlobalStringPtr("\xd8\xb5\xd8\xad\xd9\x8a\xd8\xad", "bool.print.true"); // صحيح
                    llvm::Value *falseStr = builder_->CreateGlobalStringPtr("\xd8\xae\xd8\xb7\xd8\xa3", "bool.print.false");       // خطأ
                    llvm::Value *selected = builder_->CreateSelect(v, trueStr, falseStr, "bool.print.sel");
                    builder_->CreateCall(printfFunc, {fmt, selected});
                }
                // ================================================================
                ...
                else if (v->getType()->isDoubleTy())
                {
                    ...
                    if (op.dataType == SadTypeKind::Integer)
                    {
                        ...
                        llvm::Value *asInt = builder_->CreateFPToSI(v, llvm::Type::getInt64Ty(*context_), "print.dbl2int");
                        llvm::Value *fmt = builder_->CreateGlobalStringPtr("%lld", "fmt.d");
                        builder_->CreateCall(printfFunc, {fmt, asInt});
                    }
                    else
                    {
