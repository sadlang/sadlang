...
namespace Sad
{
    namespace LLVM
    {
         ...
         */
        llvm::Value *LLVMCodeGen::emitInstructionCore(std::shared_ptr<SIRInstruction> inst)
        {
            switch (inst->opcode)
            {
            ...
            case SIROpcode::BUILTIN_LOG10:
                return emitBuiltinLog10(inst);
            case SIROpcode::BUILTIN_LOG2:
                return emitBuiltinLog2(inst);
            case SIROpcode::BUILTIN_ASIN:
                return emitBuiltinAsin(inst);
            case SIROpcode::BUILTIN_ACOS:
                return emitBuiltinAcos(inst);
            case SIROpcode::BUILTIN_ATAN:
                return emitBuiltinAtan(inst);
            case SIROpcode::BUILTIN_TRUNC:
                return emitBuiltinTrunc(inst);
            case SIROpcode::BUILTIN_FMOD:
                return emitBuiltinFmod(inst);
            case SIROpcode::BUILTIN_CLAMP:
                return emitBuiltinClamp(inst);
