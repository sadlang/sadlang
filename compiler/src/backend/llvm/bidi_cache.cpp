...
namespace sad
{
    namespace optimizer
    {
        ...
        struct BidiCharInfo
        {
            ...
            BidiCharInfo()
                : codepoint(0), bidiClass(BidiClass::L), embeddingLevel(0), isMirrored(false), mirrorCodepoint(0)
            {
            }
        };
        ...
        struct BidiAnalysis
        {
            ...
            BidiAnalysis()
                : baseDirection(TextDirection::LTR), maxEmbeddingLevel(0), hasRTL(false), hasLTR(false), hasArabic(false)
            {
            }
            ...
            static BidiClass resolve(uint32_t cp)
            {
                ...
                if (cp < 0x0080)
                {
                    if (cp >= 0x0041 && cp <= 0x005A)
                        return BidiClass::L; // A-Z
                    if (cp >= 0x0061 && cp <= 0x007A)
                        return BidiClass::L; // a-z
                    if (cp >= 0x0030 && cp <= 0x0039)
                        return BidiClass::EN; // 0-9
                    if (cp == 0x0020)
                        return BidiClass::WS; // Space
                    if (cp == 0x000A || cp == 0x000D)
                        return BidiClass::B; // Newlines
        ...
        public:
            BidiCache(size_t maxCacheSize = 10000)
                : hits(0), misses(0), maxSize(maxCacheSize)
            {
            }
