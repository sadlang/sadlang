...
namespace sad
{
    ...
    bool LLVMOptimizer::optimize(llvm::Module *module)
    {
        ...
        // (AR) إضافة LTO module flags إذا تم تفعيله
        // (EN) Add LTO module flags if enabled
        //   - ThinLTO: علامة "ThinLTO" = 1
        //   - FullLTO: علامة "EnableSplitLTOUnit" = 1
        // المرجع: LLVM Module Flags Metadata
        if (enable_lto_thin_)
        {
            module->addModuleFlag(llvm::Module::Error, "ThinLTO", 1);
            module->addModuleFlag(llvm::Module::Error, "EnableSplitLTOUnit", 1);
        }
        else if (enable_lto_full_)
        {
            module->addModuleFlag(llvm::Module::Error, "EnableSplitLTOUnit", 1);
        }
    ...
    void LLVMOptimizer::printStats() const
    {
        ...
        {
            double reduction = 100.0 * (stats_.instructions_before - stats_.instructions_after) / stats_.instructions_before;
    ...
    /**
     * (AR) ضبط وضع LTO — يحفظ الأعلام لتُستخدم عند بناء pipeline
     * (EN) Set LTO mode — flags consumed when building pipeline
     */
    void LLVMOptimizer::setLTOMode(bool enable_full, bool enable_thin)
    {
        // (AR) لا يمكن تفعيل النوعين معاً — FullLTO له الأولوية
        // (EN) Cannot enable both — FullLTO takes precedence
        enable_lto_full_ = enable_full;
        enable_lto_thin_ = enable_thin && !enable_full;
        // (AR) إعادة بناء pipeline لتطبيق الإعدادات الجديدة
        buildOptimizationPipeline(optimization_level_);
    }
    ...
    void LLVMOptimizer::buildOptimizationPipeline(OptimizationLevel level)
    {
        ...
        // (AR) إضافة Coroutine passes — مطلوبة لدعم المولّدات (generators)
        //      يجب أن تعمل على كل مستوى تحسين بما فيه O0
        //      الترتيب: CoroEarly → CoroSplit → CoroElide → CoroCleanup
        // (EN) Add Coroutine passes — required for generator support
        //      Must run at all optimization levels including O0
        module_pm_->addPass(llvm::CoroEarlyPass());

        // (AR) CoroSplit يقسم الدالة المُعلّمة بـ presplitcoroutine إلى resume/destroy
        // (EN) CoroSplit splits functions marked with presplitcoroutine into resume/destroy
        llvm::CGSCCPassManager cgpm;
        cgpm.addPass(llvm::CoroSplitPass());
        module_pm_->addPass(llvm::createModuleToPostOrderCGSCCPassAdaptor(std::move(cgpm)));

        module_pm_->addPass(llvm::createModuleToFunctionPassAdaptor(llvm::CoroElidePass()));
        module_pm_->addPass(llvm::CoroCleanupPass());
