        ...
        llvm::Value *LLVMCodeGen::resolveOperand(const SIROperand &operand)
        {
            ...
            case SIROperandType::REGISTER:
            {
                ...
                if (it != context_info_.namedValues.end() && it->second != nullptr)
                {
                    ...
                    if (operand.dataType == SadTypeKind::String)
                    {
                        ...
                        if (auto *gv = llvm::dyn_cast<llvm::GlobalVariable>(val))
                        {
                            ...
                            return builder_->CreateIntToPtr(loaded,
                                                            llvm::PointerType::getUnqual(*context_), operand.name + ".strptr");
                        }
                ...
                // namedValues is cleared on each function entry, but globalValues is never cleared
                {
                    ...
                    auto git = context_info_.globalValues.find(globalName);
                    if (git == context_info_.globalValues.end())
                    {
                        // (AR) محاولة بالاسم الأصلي (مع %)
                        // (EN) Try with original name (with %)
                        git = context_info_.globalValues.find(operand.name);
                    }
                    if (git != context_info_.globalValues.end() && git->second != nullptr)
                    {
                        ...
                        if (auto *gvInst = llvm::dyn_cast<llvm::GlobalVariable>(gval))
                        {
                            ...
                            // ================================================================
                            {
                                llvm::Value *loaded = builder_->CreateLoad(gvInst->getValueType(), gvInst, globalName + ".gload");
                                if (operand.dataType == SadTypeKind::String && loaded->getType()->isIntegerTy(64))
                                {
                                    return builder_->CreateIntToPtr(loaded,
                                                                    llvm::PointerType::getUnqual(*context_), globalName + ".strptr");
                                }
                                return loaded;
                            }
                        }
                        ...
                        {
                            return builder_->CreateLoad(allocaInst->getAllocatedType(), allocaInst, globalName + ".load");
                        }
                        return gval;
                    }
                    ...
                    if (!gv)
                    {
                        ...
                        if (stripped.size() > 4 && stripped.substr(0, 4) == "_inl")
                        {
                            ...
                            if (underscorePos != std::string::npos && underscorePos + 1 < stripped.size())
                            {
                                ...
                                gv = module_->getGlobalVariable(originalName);
                            }
                        }
                    }
                    if (gv)
                    {
                        ...
                        // ================================================================
                        {
                            llvm::Value *loaded = builder_->CreateLoad(gv->getValueType(), gv, globalName + ".gload");
                            if (operand.dataType == SadTypeKind::String && loaded->getType()->isIntegerTy(64))
                            {
                                return builder_->CreateIntToPtr(loaded,
                                                                llvm::PointerType::getUnqual(*context_), globalName + ".strptr");
            ...
            case SIROperandType::GLOBAL:
            {
                ...
                if (git != context_info_.globalValues.end() && git->second != nullptr)
                {
                    if (auto *gvInst = llvm::dyn_cast<llvm::GlobalVariable>(git->second))
                    {
                        ...
                        {
                            llvm::Value *loaded = builder_->CreateLoad(gvInst->getValueType(), gvInst, cleanName + ".val");
                            if (operand.dataType == SadTypeKind::String && loaded->getType()->isIntegerTy(64))
                            {
                                return builder_->CreateIntToPtr(loaded,
                                                                llvm::PointerType::getUnqual(*context_), cleanName + ".strptr");
                            }
                            return loaded;
                        }
                    }
                ...
                if (gv)
                {
                    ...
                    {
                        llvm::Value *loaded = builder_->CreateLoad(gv->getValueType(), gv, cleanName + ".val");
                        if (operand.dataType == SadTypeKind::String && loaded->getType()->isIntegerTy(64))
                        {
                            return builder_->CreateIntToPtr(loaded,
                                                            llvm::PointerType::getUnqual(*context_), cleanName + ".strptr");
