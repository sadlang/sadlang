...
#include "sir_optimizer/optimizer.h"
#include "sir_optimizer/constant_folding_pass.h"
#include "sir_optimizer/dead_code_elimination_pass.h"
#include "sir_optimizer/cse_pass.h"
#include "sir_optimizer/copy_propagation_pass.h"
#include "sir_optimizer/register_coalescing_pass.h"
#include "sir_optimizer/licm_pass.h"
#include "sir_optimizer/strength_reduction_pass.h"
#include "sir_optimizer/peephole_pass.h"
#include "sir_optimizer/branch_folding_pass.h"
#include "sir_optimizer/inlining_pass.h"
#include "sir_optimizer/tail_call_pass.h"
#include "sir_optimizer/sroa_pass.h"
#include <iostream>
#include <iomanip>
#include <chrono>
    ...
    namespace Compiler
    {
        namespace Optimizer
        {
            ...
            Optimizer::Optimizer(OptimizationLevel level)
                : level_(level), passManager_(std::make_unique<PassManager>()), debugMode_(false)
            {
                initializePasses();
            }
            ...
            bool Optimizer::optimize(std::shared_ptr<SIR::SIRModule> module)
            {
                ...
                // (AR) إدراج الدوال (O3 فقط) — يحتاج وصول على مستوى الوحدة
                // (EN) Function inlining (O3 only) — requires module-level access
                if (level_ >= OptimizationLevel::O3)
                {
                    InliningPass inliner;
                    inliner.setModule(module.get());
                    for (auto &func : functions)
                    {
                        if (func)
                        {
                            if (inliner.runOnFunction(func.get()))
                            {
                                modified = true;
                            }
                        }
            ...
            void Optimizer::addLevel1Passes()
            {
                ...
                // تحسينات ثقبية / Peephole Optimizations (x+0→x, x*1→x, etc.)
                addPass(std::make_unique<PeepholePass>());
            ...
            void Optimizer::addLevel2Passes()
            {
                ...
                // طي التفريعات / Branch Folding (constant branches, jump threading, dead blocks)
                addPass(std::make_unique<BranchFoldingPass>());

                // الاستبدال القيمي للتجميعات / SROA (struct → individual registers)
            ...
            void Optimizer::addLevel3Passes()
            {
                ...
                // تحسين الاستدعاء الذيلي / Tail Call Optimization (self-recursive tail calls → loops)
                addPass(std::make_unique<TailCallPass>());
