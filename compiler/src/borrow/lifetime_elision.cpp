...
namespace sad::borrow::elision
{
    ...
    struct LifetimeId
    {
        ...
        std::string toString() const
        {
            switch (kind)
            {
            case LifetimeKind::Named:
                return "'" + name;
            case LifetimeKind::Static:
                return "'ثابت";
            case LifetimeKind::Anonymous:
                return "'_";
            case LifetimeKind::Elided:
                return "";
            case LifetimeKind::Generated:
                return name;
            }
            return "";
        }
        ...
        bool operator==(const LifetimeId &other) const
        {
            if (kind != other.kind)
                return false;
            if (kind == LifetimeKind::Generated)
                        ...
                        if (lt.isGenerated() ||
                            (lt.kind == LifetimeKind::Named &&
                             std::find(sig.explicitLifetimes.begin(),
                                       sig.explicitLifetimes.end(), lt) == sig.explicitLifetimes.end()))
                        {
                            newRef.lifetime = lt;
                            break;
                        }
    ...
    public:
        ...
        std::string annotate(const FunctionSig &original, const ElisionResult &elided)
        {
            ...
            if (elided.returnType.has_value())
            {
                ...
                if (elided.returnType->isMutable)
                    ss << "متغير ";
