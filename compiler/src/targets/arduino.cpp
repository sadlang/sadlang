        ...
        namespace targets
        {
            ...
            public:
                ...
                std::string generateAvrLinkerScript()
                {
                    ...
                    script << "/* ═══════════════════════════════════════════════════════════════ */\n";
                    script << "/* " << "نص الربط لـ " << boardInfo_.name << " */\n";
                    script << "/* " << "المعالج: " << boardInfo_.mcu << " */\n";
                    script << "/* ═══════════════════════════════════════════════════════════════ */\n\n";
                ...
                std::string generateAvrVectorTable()
                {
                    ...
                    code << "; ═══════════════════════════════════════════════════════════════\n";
                    code << "; " << "جدول نواقل المقاطعات لـ " << boardInfo_.mcu << "\n";
                    code << "; ═══════════════════════════════════════════════════════════════\n\n";
                ...
                std::string generateSetupLoopWrapper()
                {
                    ...
                    code << "\n";
                    code << "  ; " << "استدعاء دالة الإعداد" << "\n";
                    code << "  call void @_Z5setupv()  ; setup()\n";
                    code << "\n";
                    code << "  ; " << "تفعيل المقاطعات" << "\n";
                    ...
                    code << "  ; " << "تهيئة المؤقت للدالة millis()" << "\n";
                ...
                std::string generatePinConstants()
                {
                    ...
                    code << "; ═══════════════════════════════════════════════════════════════\n";
                    code << "; " << "ثوابت المنافذ لـ " << boardInfo_.name << "\n";
                    code << "; Pin Constants\n";
                    code << "; ═══════════════════════════════════════════════════════════════\n\n";
                ...
                std::string generateCoreFunctions()
                {
                    ...
                    code << "; ═══════════════════════════════════════════════════════════════\n";
                    code << "; " << "دوال Arduino الأساسية" << "\n";
                    code << "; Core Arduino Functions\n";
                    code << "; ═══════════════════════════════════════════════════════════════\n\n";

                    // (AR) pinMode
                    code << "; " << "تحديد وضع المنفذ - pinMode(pin, mode)" << "\n";
                    code << "define void @pinMode(i8 %pin, i8 %mode) {\n";
                    code << "entry:\n";
                    code << "  ; " << "حساب المنفذ والبت" << "\n";
                    code << "  %port_idx = udiv i8 %pin, 8\n";
                    code << "  %bit_pos = urem i8 %pin, 8\n";
                    code << "  %bit_mask = shl i8 1, %bit_pos\n";
                    code << "  ; " << "تعيين اتجاه المنفذ" << "\n";
                    code << "  ; ... (implementation)\n";
                    code << "  ret void\n";
                    code << "}\n\n";

                    // (AR) digitalWrite
                    code << "; " << "كتابة رقمية - digitalWrite(pin, value)" << "\n";
                    code << "define void @digitalWrite(i8 %pin, i8 %value) {\n";
                    code << "entry:\n";
                    code << "  %port_idx = udiv i8 %pin, 8\n";
                    code << "  %bit_pos = urem i8 %pin, 8\n";
                    code << "  %bit_mask = shl i8 1, %bit_pos\n";
                    code << "  ; " << "تعيين قيمة المنفذ" << "\n";
                    code << "  ret void\n";
                    code << "}\n\n";

                    // (AR) digitalRead
                    code << "; " << "قراءة رقمية - digitalRead(pin)" << "\n";
                    code << "define i8 @digitalRead(i8 %pin) {\n";
                    code << "entry:\n";
                    code << "  %port_idx = udiv i8 %pin, 8\n";
                    code << "  %bit_pos = urem i8 %pin, 8\n";
                    code << "  %bit_mask = shl i8 1, %bit_pos\n";
                    code << "  ; " << "قراءة قيمة المنفذ" << "\n";
                    code << "  ret i8 0\n";
                    code << "}\n\n";

                    // (AR) analogRead
                    code << "; " << "قراءة تماثلية - analogRead(pin)" << "\n";
                    code << "define i16 @analogRead(i8 %pin) {\n";
                    code << "entry:\n";
                    code << "  ; " << "تحديد قناة ADC" << "\n";
                    code << "  ; " << "بدء التحويل" << "\n";
                    code << "  ; " << "انتظار الانتهاء" << "\n";
                    code << "  ; " << "قراءة النتيجة" << "\n";
                    code << "  ret i16 0\n";
                    code << "}\n\n";

                    // (AR) analogWrite (PWM)
                    code << "; " << "كتابة تماثلية (PWM) - analogWrite(pin, value)" << "\n";
                    code << "define void @analogWrite(i8 %pin, i8 %value) {\n";
                    code << "entry:\n";
                    code << "  ; " << "تهيئة PWM للمنفذ" << "\n";
                    code << "  ; " << "تعيين دورة العمل" << "\n";
                    code << "  ret void\n";
                    code << "}\n\n";

                    // (AR) delay
                    code << "; " << "تأخير بالمللي ثانية - delay(ms)" << "\n";
                    code << "define void @delay(i32 %ms) {\n";
                    code << "entry:\n";
                    code << "  ; " << "استخدام المؤقت للتأخير" << "\n";
                    code << "  ret void\n";
                    code << "}\n\n";

                    // (AR) millis
                    code << "; " << "الوقت منذ البدء بالمللي ثانية - millis()" << "\n";
                    ...
                    // (AR) micros
                    code << "; " << "الوقت منذ البدء بالمايكرو ثانية - micros()" << "\n";
                    code << "define i32 @micros() {\n";
                    code << "entry:\n";
                    code << "  ; " << "حساب الوقت من المؤقت" << "\n";
                    code << "  ret i32 0\n";
                    code << "}\n\n";
