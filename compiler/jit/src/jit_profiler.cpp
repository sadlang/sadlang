...
ProfileScope::ProfileScope(const std::string& function_name, const std::string& phase)
    : function_name_(function_name)
    , phase_(phase)
{
    entry_.function_name = function_name_;
    entry_.phase = phase_;
    entry_.thread_id = JITProfiler::getCurrentThreadId();
    entry_.start_time = std::chrono::steady_clock::now();
    
    // قياس الذاكرة الأولية / Measure initial memory
    entry_.memory_used_bytes = getCurrentMemoryUsage();
}

ProfileScope::~ProfileScope() {
    entry_.end_time = std::chrono::steady_clock::now();
    
    // حساب المدة / Calculate duration
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
        entry_.end_time - entry_.start_time);
    entry_.duration_us = duration.count();
    
    // حساب استهلاك الذاكرة / Calculate memory consumption
    size_t final_memory = getCurrentMemoryUsage();
    if (final_memory > entry_.memory_used_bytes) {
        entry_.memory_used_bytes = final_memory - entry_.memory_used_bytes;
    } else {
        entry_.memory_used_bytes = 0;
    }
    
    // تسجيل الإدخال / Record entry
    JITProfiler::getInstance().recordEntry(entry_);
}

void ProfileScope::addMetadata(const std::string& key, const std::string& value) {
...
size_t ProfileScope::getCurrentMemoryUsage() {
#ifdef _WIN32
