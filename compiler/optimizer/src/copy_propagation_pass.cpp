...
#include "../include/copy_propagation_pass.h"
...
bool CopyPropagationPass::processBlock(SIR::SIRBasicBlock* block, int blockIndex) {
    ...
    for (auto& inst : block->instructions) {
        auto copyInfo = detectCopy(inst.get(), blockIndex);
        if (copyInfo) {
    ...
    for (const auto& copy : copies_) {
        ...
        if (replacements > 0) {
            ...
            if (!isRegisterUsed(block->getParent(), copy.destination, blockIndex)) {
...
std::unique_ptr<CopyInfo> CopyPropagationPass::detectCopy(
    ...
    if (inst->result->kind == SIR::OperandKind::REGISTER) {
        destination = inst->result->value;
    } else {
        return nullptr;
    }
    ...
    if (firstOperand.kind == SIR::OperandKind::REGISTER) {
        source = firstOperand.value;
    } else {
        return nullptr;
    }
...
bool CopyPropagationPass::isSimpleCopy(const SIR::SIRInstruction* inst) const {
    ...
    if (inst->operands.size() >= 2) {
        ...
        if ((inst->opcode == SIROpcode::ADD_I64 || inst->opcode == SIROpcode::ADD_F64) &&
            secondOperand.kind == SIR::OperandKind::IMMEDIATE) {
            if (secondOperand.value == "0" || secondOperand.value == "0.0") {
                return true;
            }
        }
        ...
        if ((inst->opcode == SIROpcode::MUL_I64 || inst->opcode == SIROpcode::MUL_F64) &&
            secondOperand.kind == SIR::OperandKind::IMMEDIATE) {
            if (secondOperand.value == "1" || secondOperand.value == "1.0") {
                return true;
            }
        }
...
bool CopyPropagationPass::removeCopyInstruction(
    ...
    auto it = std::find_if(instructions.begin(), instructions.end(),
        [inst](const std::unique_ptr<SIR::SIRInstruction>& ptr) {
            return ptr.get() == inst;
        });
