...
#include "../include/dead_code_elimination_pass.h"
#include <queue>
#include <algorithm>
...
DeadCodeEliminationPass::computeReachableBlocks(SIR::SIRFunction* function) {
    ...
    const auto& blocks = function->getBasicBlocks();sicBlocks();
    if (blocks.empty()) {
        return reachable;
    }
...
bool DeadCodeEliminationPass::hasSideEffect(SIR::SIRInstruction* inst) const {
    ...
    switch (opcode) {
        ...
        case SIR::SIROpcode::PRINT:  // إذا كان موجوداً / If exists
            return true;
