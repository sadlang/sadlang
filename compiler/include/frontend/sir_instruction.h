    ...
    namespace Compiler
    {
        namespace SIR
        {
            ...
            public:
                ...
                // ─── بيانات SIMD المتجهات (Phase 3) / SIMD vector metadata ───
                // (AR) لتعليمات VECTOR_*: نوع العنصر وعدد lanes.
                //      مثال: vectorElementType=Float32, vectorLaneCount=8 → <8 x float>
                // (EN) For VECTOR_* instructions: element type and lane count.
                int vectorElementType = 0;    ///< (cast SadTypeKind to int) — عند 0 يُعتبر غير محدد
                int vectorLaneCount = 0;      ///< 2, 4, 8, 16, 32, 64
                std::vector<int> shuffleMask; ///< قناع الخلط لـ VECTOR_SHUFFLE
