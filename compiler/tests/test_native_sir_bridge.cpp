...
namespace
{
    ...
    // (AR) أصغر بمعاملٍ عشريٍّ ⇒ يجب أن يرفضَه الحارسُ (res.ok==false) بدلَ إنتاجِ مقارنةِ بتّاتٍ خاطئة.
    const std::string kSrcMinFloatRejected =
        "\xD8\xAF\xD8\xA7\xD9\x84\xD8\xA9\x20\xD8\xB1\xD8\xA6\xD9\x8A\xD8\xB3\xD9\x8A\xD8\xA9\x28\x29\x0A"
        "\x20\x20\x20\x20\xD8\xA7\xD8\xB1\xD8\xAC\xD8\xB9\x20\xD8\xA3\xD8\xB5\xD8\xBA\xD8\xB1\x28\x31\x2E\x35\xD8\x8C\x20\x32\x2E\x35\x29\x0A"
        "\xD9\x86\xD9\x87\xD8\xA7\xD9\x8A\xD8\xA9\x0A";
} // namespace
...
TEST(NativeSirBridge, RejectsFloatMinMax)
{
    auto module = buildSir(kSrcMinFloatRejected);
    ASSERT_TRUE(module != nullptr);
    auto rx = sad::native::lowerModuleToElf(*module);
    ASSERT_TRUE(!rx.ok); // x86 يرفض العشريّ
    auto ra = sad::native::lowerModuleToElfArm64(*module);
    ASSERT_TRUE(!ra.ok); // ARM64 يرفض العشريّ (تماثل)
}
