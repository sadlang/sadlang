// ═══════════════════════════════════════════════════════════════════
// صNet — تنفيذ التوجيه البصلي
// (AR) بناء حلقات بصلية، تغليف/فك طبقات التشفير
//      عُقدة ترحيل، مدير توجيه مجهول الهوية
// (EN) Onion routing implementation: circuits, relay, anonymous router
// المكون: features/network/sadnet (التوجيه البصلي)
// المسار: features/network/sadnet/src/routing/onion_routing.cpp
// ═══════════════════════════════════════════════════════════════════
...
namespace sad::net::routing
{
    ...
    Result<Buffer> Circuit::wrap(const Buffer &plaintext) const
    {
        ...
        // تغليف من القفزة الأخيرة إلى الأولى
    ...
    // ═════════════════════════════════════════════════════════════════════
    // (AR) باني الحلقات — تبادل مفاتيح X25519 تدريجي
    // ═════════════════════════════════════════════════════════════════════
    ...
    struct CircuitBuilder::Impl
    {
        ...
        // إنشاء قفزة واحدة عبر تبادل مفاتيح
        Result<CircuitHop> create_hop(const NodeId &node_id,
                                      const Endpoint &ep,
                                      uint32_t circuit_id)
        {
            // إنشاء زوج مفاتيح مؤقت
            auto ephemeral = crypto::x25519_generate_keypair();

            // بناء خلية CREATE: [circuit_id:4][public_key:32]
            OnionCell cell;
    ...
    // ═════════════════════════════════════════════════════════════════════
    // (AR) عُقدة الترحيل البصلية
    //      تحتفظ بجدول حلقات نشطة، تفك طبقة تشفير وتعيد التوجيه
    // ═════════════════════════════════════════════════════════════════════
                                         ...
                                         RelayForwarder forwarder)
    {
        ...
        {
            // تبادل مفاتيح: استقبال المفتاح العام وإرسال مفتاحنا
            ...
            // إرسال CREATED مع مفتاحنا العام
            OnionCell resp;
        ...
        if (cell.type == CellType::RELAY || cell.type == CellType::DATA)
        {
            ...
            // إعادة توجيه إلى القفزة التالية
    ...
    Result<Buffer> OnionRouter::send_anonymous(const Endpoint &destination,
                                               ...
                                               CellSender sender)
    {
        // يحتاج قائمة من العُقد المتاحة لبناء المسار
        // في الاستخدام الحقيقي، يُستدعى select_path أولاً ثم create_circuit
        return Result<Buffer>::fail(NetError::NOT_FOUND);
    }
