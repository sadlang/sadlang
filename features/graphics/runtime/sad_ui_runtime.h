...
/** مربع_دوار() */ SadWidget sad_spin_box(void);
/** صندوق_تجميع() */ SadWidget sad_group_box(void);
/** مؤشر_انشغال() */ SadWidget sad_spinner(void);
/** شريط_حالة() */ SadWidget sad_status_bar(void);
...
/** التقط_مفتاح() — يعيد رمز أوّل مفتاحٍ منتظِرٍ (UnifiedKeyCode كعدد) ويستهلكه، أو 0
 *  إن كان طابور المفاتيح فارغًا. يُستدعى من البانِي كلّ إعادة بناءٍ في لولبٍ لاستنزاف
 *  المفاتيح المنتظِرة ⇒ تطبيقاتٌ نصّيّة تفاعليّة (طرفيّة/حقول) بلغة ص خالصة. الحلقة
 *  الحرّة تُلحِق كلّ مفتاحٍ (عدا F2 للخروج) وتوسّخ العرض. يعيد 0 حيث لا مصدر إدخالٍ حرّ. */
int sad_next_key(void);
