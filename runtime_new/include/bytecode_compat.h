 ...
 * @details
 * ═══════════════════════════════════════════════════════════════════════════
 * (AR) شرح موسّع — طبقة التوافق بين runtime_new والمترجم
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * المشكلة الجوهرية:
 * ─────────────────
 * مكتبة runtime_new كُتبت في الأصل لتعمل مع واجهة برمجية (API) افتراضية
 * لم تُنفَّذ أبداً في المترجم الفعلي. المترجم الحالي (compiler_new) يستخدم
 * فضاءات أسماء وأنواع مختلفة تماماً:
 *
 *   ┌──────────────────────────┬──────────────────────────────────────┐
 *   │ ما يتوقعه runtime_new    │ ما يوجد فعلاً في compiler_new       │
 *   ├──────────────────────────┼──────────────────────────────────────┤
 ...
 * - 30+ opcode aliases mapping old runtime_new names to compiler names
 ...
 *
 * @note هذا الملف حرج لعمل مكتبة runtime_new بأكملها. أي تغيير في
 *       opcodes.h أو bytecode.h قد يتطلب تحديث الأسماء البديلة هنا.
 * @note This file is critical for the entire runtime_new library. Any
 *       changes to opcodes.h or bytecode.h may require alias updates here.
 *
 * @see runtime_new/include/vm/vm.h — الآلة الافتراضية التي تستهلك هذه الأسماء
 * @see compiler_new/include/bytecode/opcodes.h — مصدر أكواد العمليات الأصلية
 * @see compiler_new/include/bytecode/bytecode.h — مصدر BytecodeModule
 *
...
//      لكن runtime_new يستخدمها. نُخصص لها قيم فارغة في مجال 0xC0-0xEF.
// (EN) These opcodes don't exist in the compiler's current instruction set,
//      but runtime_new uses them. We assign unused values in 0xC0-0xEF range.
// ====================================================================
 ...
 *      تحت الاسم المختصر Sad::Bytecode الذي يستخدمه runtime_new/
 * (EN) Provides direct access to compiler types from Sad::Compiler::Bytecode
 *      under the short name Sad::Bytecode used by runtime_new/
 */
namespace Bytecode {
