 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * ملف: allocator.h
 * الوصف: نظام تخصيص الذاكرة المتقدم للغة ص
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * الغرض من هذا الملف:
 * ──────────────────
 * يوفر هذا الملف نظام تخصيص ذاكرة عالي الأداء مصمم خصيصاً لوقت تشغيل
 * لغة ص. يستخدم تقنية تجمعات الذاكرة (Memory Pools) لتحسين سرعة التخصيص
 * والتحرير إلى O(1) بدلاً من O(log n) في malloc العادية.
 *
 * المكونات الرئيسية:
 * ─────────────────
 * - BlockHeader: رأس كل كتلة ذاكرة يحمل البيانات الوصفية (الحجم، المعرّف،
 *   الأعلام، الرقم السحري للتحقق من صحة الذاكرة)
 * - MemoryPool: تجمع ذاكرة لحجم كتلة محدد. يحتفظ بقائمة حرة (free list)
 *   للتخصيص السريع. يتوسع تلقائياً عند الحاجة.
 * - MemoryAllocator: النظام الرئيسي (Singleton) الذي يدير عدة تجمعات:
 *   • تجمع صغير (16 بايت) — للأعداد والقيم المنطقية
 *   • تجمع متوسط (64 بايت) — للنصوص القصيرة
 *   • تجمع كبير (256 بايت) — للمصفوفات الصغيرة
 *   • تجمع ضخم (1024 بايت) — للكائنات والقواميس
 *   • تخصيص عام — لأي حجم أكبر من 1024 بايت
 *
 * الموقع في البنية العامة:
 * ───────────────────────
 *   runtime_new/
 *   ├── memory/
 *   │   ├── [allocator.h — هذا الملف] ← نظام التخصيص
 *   │   └── gc.h ← جامع القمامة (يعتمد على المخصص)
 *   ├── vm/ ← الآلة الافتراضية (تستخدم المخصص لإنشاء القيم)
 *   └── thread/ ← الخيوط (المخصص آمن للتزامن)
 *
 * الاعتماديات:
 * ──────────
 * - <atomic>, <mutex>: للسلامة في بيئة الخيوط المتعددة
 * - <vector>, <unordered_map>: لتخزين الصفحات والإحصائيات
 * - gc.h يعتمد على هذا الملف (وليس العكس)
 *
 * ميزات متقدمة:
 * ────────────
 * - محاذاة الذاكرة (8 بايت افتراضياً، 64 بايت لخطوط الكاش)
 * - كشف تسرب الذاكرة في وضع التصحيح (SAD_DEBUG_MEMORY)
 * - رقم سحري (0xDEADBEEF) للتحقق من سلامة الكتل
