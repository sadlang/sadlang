...
SOT_CHECK_GUARDS = (
    ...
    {
        # (AR) قارئا العقدِ يقولانِ الشيءَ نفسَه. عقدُ البذرةِ وسمٌ في تعليق،
        #      ويقرؤه في هذه الشجرةِ قرّاءٌ عدّةٌ مكتوبونَ باليدِ في ملفّاتٍ شتّى،
        #      وليس فيها مَن يقيسُ اتّفاقَهم — فينجرفُ أحدُهم عن الآخرِ صامتًا.
        #      والانجرافُ مقيسٌ لا مُتخيَّل: ثلاثةُ قرّاءٍ أنتجوا ثلاثةَ أرقامٍ
        #      متناقضةٍ لعدَّادٍ واحد؛ ونسخةٌ بـ`\s` جعلت الحارسَ ٧٧ والمقياسَ ٧٨؛
        #      و`utf-8` مقابلَ `utf-8-sig` جعلَ الحارسَ يرى عقدًا والعدّاءَ يتخطّى.
        #      ولامتغيِّراتُه: خلافُ الحدِّ بينَ العدّاءِ (`tests/runner.py` —
        #      **مُنفِّذُ العقد**) والحارسِ على كلِّ بذرةٍ (**صفر**) · عددُ
        #      الأنماطِ المُصرَّفةِ المكتوبةِ باليد (محدودٌ من طرفَيه: الانكماشُ
        #      يُحمِّرُ ولا يُقرأُ تقدّمًا) · خلافُ قرّاءِ `@rule` بموضعِ كلٍّ
        #      منهم ورايتِه (**صفر**) · ملفّاتٌ لا تُحلَّلُ نحويًّا (سقفٌ نازل) ·
        #      وترميزُ **قارئِ نصِّ البذرة** (`utf-8-sig` — **صفر**؛ وردُّه إلى
        #      `utf-8` أعادَ أربعَ بذورٍ ذاتِ بادئةٍ إلى «بلا وسم @rule» وهي
        #      تحملُه) · وأرضيّتانِ تُقاسانِ لا تُفترَضان: ⑦ نطاقُ التخطّي
        #      المُشتقُّ من `.gitignore` و⑧ عددُ قرّاءِ نصِّ البذرة — فصفرُ
        #      قارئٍ لا يُقرأُ «صفرَ مخالفة».
        #      ⚠️ و⑤ يقيسُ **فعلَ قراءةِ نصِّ بذرةٍ** لا شكلَ حلقةٍ بعينِه:
        #         الملفُّ يُصرِّفُ وسمًا من المعجم، والنداءُ في حلقةِ أسطرٍ أو في
        #         دالّةٍ تُطبِّقُ وسمًا. وقارئٌ يقرأُ البذرةَ نصًّا كاملًا خارجَ
        #         هذَين خارجَ المدى — دَينٌ مُسمًّى.
        #      ⚠️ و① يقيسُ **طرفَي العقدِ المُسمَّيَينِ وحدَهما** لا كلَّ قارئٍ في
        #         الشجرة؛ وصورُ القراءةِ غيرِ المُصرَّفةِ (`startswith` · `in`)
        #         خارجَ اللامتغيِّراتِ الخمسةِ كلِّها — دَينٌ مُسمًّى.
        #      والعدّاءُ يُستوردُ ولا يُغيَّرُ: الحارسُ يعتمدُ عليه، لا العكس.
        # (EN) The runner (the contract's executor) and the guards must give the
        #      same answer on every seed. Imports tests/runner.py read-only.
        "name": "seed_tag_readers",
        "script": "check_seed_tag_readers.py",
        "args": (),
    },
)
